...
> ⚠️ Bu belge kalemlerin TANIMINI tutar, DURUMUNU TUTMAZ. Buradaki işaretler 2026-08-27 durumudur. Güncel durum: `docs/otonom/00-KUYRUK.md` (köprü: `docs/kararlar/00-KART-INDEKSI.md`)
...
> ⚠️ **GÜNCELLEME (2026-09-02, G1 ÇAPRAZ DOĞRULAMA — kart↔kod, salt-okuma doğrulama turu):** Kartlar 2026-08-27'de dondurulduktan sonra Faz 1-4 turlarında iş yapıldı ama kart durumları güncellenmemişti. Bu turda 30/30 kart KODDAN doğrulandı (dosya:satır kanıtlı); durumlar kart-bazında ⚠️ notlarla güncellendi (**gövde metni SİLİNMEDİ — eski durum üstü-çizili `[ESKİ]` damgalı**, G9-03 deseni). Kod DEĞİŞMEDİ (yalnız-belge). **🔴 HAYALET (✅ ama yapılmamış) = 0.**
> - ⭐ **📸 ETİKET SORUSU (PO KARARINA):** Bu dosya "dondurulmuş" ama durum güncellemeleri alıyor (2026-08-28'den beri satır-içi notlar zaten vardı) → **hibrit.** Öneri: gövde 2026-08-27 snapshot'ı kalır (📸), durum güncellemeleri ⚠️ notlarla eklenir; ya da 🔄 YAŞAYAN'a çevrilir. **Etiket değişimi = KURAL 3/10 konusu → PO seçsin** (bu turda unilateral değiştirilmedi).
> - ⭐ **İŞ 0 — İKİ NUMARA ÇAKIŞMASI (PO KARARINA, numara VERİLMEDİ):** aşağıda **G1-19** ve **G1-23** kartlarında `⚠️ ÇAKIŞMA` notu. İkisi de **FARKLI KONU** (aynı numara iki ayrı bulguda kullanılmış) → birleştirilmedi, çözülmedi.
...
~~[ESKİ · 2026-08-27 snapshot — aşağıdaki güncel tabloyla değişti]~~
...
⚠️ **GÜNCELLEME (2026-09-02, G1 çapraz-doğrulama — kod-kanıtlı):**

| durum | adet | kart no'ları | (2026-08-27'den değişim) |
|---|:---:|---|---|
...
Bugünkü durum: ~~[ESKİ · 2026-08-27] ⬜ AÇIK~~ → ⚫ KAPSAM DÜŞÜRÜLDÜ (2026-09-02; orijinal talep ≠ yapılan, ↓)
...
~~[ESKİ · 2026-08-28 çerçeve] ✅ YAPILDI (metin)~~ → ⚫ **KAPSAM DÜŞÜRÜLDÜ (PO kararı 2026-08-28; çerçeve düzeltildi 2026-09-02, G1 çapraz-doğrulama):** ⚠️ "yaptık" DEĞİL, "yapmamaya karar verdik" — orijinal talep (**doğum tarihi alanı + DB kolonu**) BİLİNÇLİ olarak DÜŞÜRÜLDÜ; yerine **tek-kutu metin beyanı** kondu (migration YOK). Kod-kanıt: `birthDate`/`birthYear`/`dogumTarihi` backend+frontend+schema geniş arandı → **0 sonuç** (gerçek yaş alanı YOK); metin `_RegisterContent.tsx:410-416` + STK `Step4Account.tsx:199-208` (18+ öne çıkarıldı). Hukuki koruma aydınlatma metnindeki **AÇIK BEYAN**'a devredildi → **G1-10** avukat paketi. **18+ = beyan (rıza değil); consent tablosuna girmez** (`konu/consent-modeli-plani-2026-08-28.md`). ⚠️ **YAŞ VERİSİ SİSTEMDE YOK** — ileride biri "yaş verimiz var" SANMASIN. 18-altı kabul kararı gelirse gerçek yaş + veli onayı ZORUNLU olur → yeniden açılır.
---
...
Bugünkü durum: ~~[ESKİ · 2026-08-27] ❓ TEYİT GEREK~~ → ✅ TEYİT (2026-09-02, kanıt ↓)
...
✅ **TEYİT (2026-09-02, G1 çapraz-doğrulama — kod-kanıtlı): SIZINTI YOK.** `matchingController.ts:73-80` — `buildMentiFacingMentorItem` çıktısında **discType YOK, discScore YOK**, jenerik "İletişim tarzları uyumlu" metni (kod-yorumu: "DISC harfi/tipi sızmaz"). Faz 3c'de de teyit (analyticsRoutes requireSelfOrAdmin + counterpart select disc'siz). ❓ → **✅ TEYİT.**
...
Bugünkü durum: ~~[ESKİ · 2026-08-27] ⬜ AÇIK~~ → ⚫ GEÇERSİZ (2026-09-02; yeniden tanım — public tasarım, izolasyon açığı DEĞİL ↓)
...
⚠️ bilanço doğrulandı: `schema.prisma` `SuspicionReport` modelinde `tenantName` var, `tenantId` YOK (geniş teyit edildi).
⚫ **GEÇERSİZ — YENİDEN TANIM (karar 2026-08-29 Faz 3b; kart-güncelleme 2026-09-02, kod-kanıtlı):** `tenantId` gerçekten YOK (kart olgusal doğru) AMA sonraki karar bunu **tenant-izolasyon açığı DEĞİL** diye yeniden tanımladı: SuspicionReport = **public davet-şüphesi formu, user ID yok, platform-only-read** (bilinçli tenant-bağımsız). Kanıt: `schema.prisma` yorumu *"SuspicionReport (public davet-şüphesi, user ID yok) ... bu tenant-scoped DEĞİL"* + `00-KARAR-TAKIP` Faz 3b "G1-04 → yeniden tanım: tenant açığı değil (tasarım)". Spam sertleştirme = ayrı UX işi (G1-26). ⬜ → **⚫ (iş kalemi değil).**
...
Bugünkü durum: ~~[ESKİ · 2026-08-27] 🟡 YARIM~~ → ✅ YAPILDI (2026-09-02, kanıt ↓)
...
✅ **YAPILDI (2026-09-02, G1 çapraz-doğrulama — kod-kanıtlı; iş backend PR #59):** Self-servis KVKK hak akışı CANLI. `GET /api/me/data-export` (userId TOKEN'dan → IDOR yapısal imkânsız; profil+rızalar+mesaj SAYISI, içerik yok) + `POST /api/me/delete-account` (e-posta teyidi → `hardDeleteUser` anonimleştirir; son-admin guard `isSoleActiveTenantAdmin` 409; ACIK_RIZA `revokeConsent`). Rate-limit `rateLimiter.ts:202-216` (per-user 5/dk). Test `me-data-rights.test.ts`. FE `DataPrivacySection` (indir + iki-adımlı onay). **madde 97 kapandı.** 🟡 → **✅.** ⚠️ Kalan H-9 (userId-pseudonim) hukukçuda — bu kart kapsamı dışı.
...
Bugünkü durum: ~~[ESKİ · 2026-08-27] ⬜ AÇIK~~ → 🟡 YARIM (2026-09-02; FeedbackLog ✅, Message karara-bağlı ↓)
...
✅ **KOD-KANIT (2026-09-02, G1 çapraz-doğrulama):** `gdprService.ts:342 FEEDBACK_LOG_RETENTION_YEARS=3` + `:371 feedbackLog.deleteMany` (haftalık cron `cronScheduler.ts:84`) → FeedbackLog 3-yıl imhası CANLI. `:375-377` açık yorum: *"TODO(G1-10): Message imha kodu BİLİNÇLİ YAZILMADI"* (keyfi süre metinle çelişir). Dürüst durum: **FeedbackLog ✅ · Message ⬜ (karara-bağlı bekliyor, yapılmamış DEĞİL).**
...
Bugünkü durum: ~~[ESKİ · 2026-08-27] ⬜ AÇIK~~ → ✅ YAPILDI (2026-09-02 kart-güncelleme; iş CANLI 2026-08-28, kanıt ↓ + satır-içi Tur A/B1/B2)
...
~~[ESKİ · 2026-08-27] ⚠️ bilanço doğrulandı: `consentVersion`/`policyVersion` hem backend hem frontend hem schema.prisma'da geniş arandı → yalnız belgelerde geçiyor, kodda YOK.~~
⚠️ **GÜNCELLEME (2026-09-02, kod-kanıtlı — artık YANLIŞ):** Consent modeli KODDA VAR → `schema.prisma:1293 model Consent` + `:1299 version` alanı · `consentService.ts:28 CONSENT_VERSION='v1.0'`. "kodda YOK" iddiası eskidi.

[x] işleme al   [ ] şimdilik alma   [ ] geçersiz   [ ] anlamadım / açıkla
[x] PO notu: 🔴 ÇIKIŞ BLOKERİ — çıkış öncesi tamamlanmalı.
✅ **YAPILDI (Tur A + B1 + B2 TAMAM) — GÜNCELLEME (2026-08-28, PR #134):** Consent modeli **CANLIDA tam devrede.** **B2 backfill `--apply`:** 5 ACIK_RIZA satırı yazıldı (5 user + 0 tenant). Doğrulama: Consent=5 (yalnız ACIK_RIZA, **AYDINLATMA YOK**) · source=BACKFILL, version=v1.0-legacy, hepsi aktif (revokedAt null) · **grantedAt==kvkkConsentAt 5/5** · User/Tenant/kvkkConsentAt DEĞİŞMEDİ (6/2/5/0) · idempotens teyitli (ikinci dry-run=0). **⬜ Kalan (bu karta bağlı DEĞİL, ayrı):** `CONSENT_VERSION` avukat metniyle sabitlenmeli (**G1-10**) · **G1-08** OAuth rıza UI + tipli kutu (servis hazır) · **G1-05** self-servis KVKK hak ekranı. *(Tur B1/A geçmişi ↓)*
🟡 **B1 (migration CANLIDA) — GÜNCELLEME (2026-08-28, PR #133):** **Consent migration CANLI Neon'a UYGULANDI** (`db execute` + `migrate resolve --applied`; `migrate status` = up to date). Doğrulama: tablo+9 sütun+2 enum+3 index+2 FK · **ön-sayımlar DEĞİŞMEDİ** (6/2/5/0) · Consent = 0 (boş). Backfill DRY-RUN: 5 satır. ⏬ *(Tur A geçmişi ↓)*
...
Nerede durdu: ~~[ESKİ · 2026-08-27] Migration + hukukçu sonrası; kısmen başlanmış.~~ ⚠️ GÜNCELLEME (2026-09-02): "kısmen başlanmış" iddiası kod-kanıtsız (↓) — hiç başlanmamış, planlı.
Bugünkü durum: ~~[ESKİ · 2026-08-27] 🟡 YARIM~~ → ⬜ AÇIK (2026-09-02, TERS DÜZELTME — kod-kanıtsız, ↓)
...
⬜ **TERS DÜZELTME (2026-09-02, G1 çapraz-doğrulama — KOD HAKEM):** Kart "🟡 kısmen başlanmış" diyor ama **kodda hiçbir yasal-alan/akış izi YOK.** KURAL 13 kapsam beyanı (iki-dilli, **11 terim · schema.prisma + tüm-repo tarandı, 0 sonuç**): `legalName·taxNo·VERBIS·dpaSigned·dataProcessing·kepAddress·mersisNo` (İng.) + `unvan·vergi·sözleşme·işleyen` (Türkçe). Tenant modelinde yasal-kimlik alanı yok. Kod ↔ kart çelişti → **KOD KAZANIR: 🟡 → ⬜** (hiç başlanmamış, planlı; madde 90 hukukçu onayı sonrası).
...
Nerede durdu: ~~[ESKİ · 2026-08-27] Backfill kodu yok~~; ürün sahibi kararıyla EN SON'a ertelenmiş.
Bugünkü durum: ~~[ESKİ · 2026-08-27] ⬜ AÇIK~~ → 🟡 YARIM (2026-09-02; teknik backfill ✅, K3 politika açık ↓)
...
🟡 **YARIM — GÜNCELLEME (2026-09-02, G1 çapraz-doğrulama — kod-kanıtlı):** Kartın "backfill kodu yok" iddiası ARTIK YANLIŞ. `backend/scripts/backfill-consent.ts` VAR ve `--apply` ÇALIŞTI (2026-08-28, PR #134, CANLI): **5 ACIK_RIZA satırı** yazıldı (5 user + 0 tenant), `grantedAt==kvkkConsentAt 5/5`, idempotens teyitli. → Teknik backfill (mevcut rızaları Consent'e taşıma) ✅. **⬜ Kalan (K3):** rıza mekanizması ÖNCESİ kayıtlar için **yeniden-rıza politikası** (bulk/erteleme) — PO ürün+hukuk kararı, "EN SON"a ertelendi. Karar-bağlı bekliyor.
...
Bugünkü durum: ~~[ESKİ · 2026-08-27] ⬜ AÇIK~~ → ⚫/✅ (2026-09-02; başka yerde çözüldü — middleware.ts VAR + admin-guard backend'de ↓)
...
~~[ESKİ · 2026-08-27] ⚠️ bilanço doğrulandı: `frontend/src/middleware.ts` glob ile arandı → dosya YOK.~~
⚫/✅ **BAŞKA YERDE ÇÖZÜLDÜ — YENİDEN TANIM (karar 2026-08-29 Faz 3a/3b; kart-güncelleme 2026-09-02, kod-kanıtlı):** ⚠️ "yapılmadı" DEĞİL. (1) `frontend/src/middleware.ts` **ARTIK VAR** (Faz 3a — www→301; glob teyit) → kartın "dosya YOK" iddiası eskidi. (2) Admin-guard: cross-origin cookie nedeniyle (frontend `sivilkapasite.org` ↔ backend `api.sivilkapasite.org`, parent-domain paylaşımı yok) middleware ile **ÇÖZÜLEMEZ** → JS-yazılabilir rol çerezi REDDEDİLDİ (sahte güven). Gerçek koruma **backend yetki denetimine** taşındı: Faz 3b'de 187 endpoint denetlendi + `platform/layout.tsx` istemci guard eklendi (`00-KARAR-TAKIP` "G1-17 → ✅"). ⬜ → **⚫ (literal talep geçersiz) / ✅ (endişe backend'de çözüldü).**
...
Bugünkü durum: ~~[ESKİ · 2026-08-27] ❓ TEYİT GEREK~~ → ✅ TEYİT (2026-09-02, okuma kaynağı doğrulandı, kanıt ↓)
...
🟡 **YARIM — GÜNCELLEME (2026-09-02, G1 çapraz-doğrulama — kod-kanıtlı):** Kartın "IP-limit yok" iddiası ARTIK YANLIŞ. `suspicionRoutes.ts:9` → `router.post('/', suspicionReportRateLimiter, createSuspicionReport)` — şüphe formunda IP-bazlı rate-limit KODDA VAR (`rateLimiter.ts`). **⬜ Kalan:** CAPTCHA + hassas işlemde step-up şifre (yapılmadı). Dürüst durum: **IP-limit ✅ · CAPTCHA/step-up ⬜.** *(Not: register'daki `selfServeRegisterRateLimiter` AYRI bir uç — Faz 3c.)*
