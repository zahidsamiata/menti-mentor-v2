...
>
> **Son güncelleme:** 2026-08-19 (merge turu — **#7 Aşama 1 ✅ MERGED, canlıda**: değerlendirme/metrik ölü uçları bağlandı (backend #48 + çatı #100 + bu docs PR). Çatı `ef2b995` · backend `b5f4b88` · pointer senkron · açık kod PR 0/0. Detay: `09-DURUM.md` "✅ #7 AŞAMA 1". **Aşama 2/3 açık** (otomatik pasifleştirme=migration, yeniden değerlendirme, ContextualFeedbackHost).
...
6. **Onay paneli bildirim maili** (S-M) — mail altyapısı hazır; kurum onay/ret maili + `destek@` + prod `PLATFORM_ADMIN_EMAIL` bağlanacak (başvuran sessiz kalmasın).
   ...
   > ⚠️ GÜNCELLEME (2026-08-19, merge turu): **düzeltme-iste akışı ✅ MERGED, CANLIDA** — backend #50 (`ba92dfa`) + çatı #104 (`2639e2e`) merge; canlı DB'de enum+kolon teyitli. "Düzeltme iste" kısmı KAPANDI. **Mail GÖNDERİMİ hâlâ KAPALI** → madde **37m** (PO env) açık; kurum onay/ret **maili** bu madde (6) kapsamında hâlâ gönderilmez (bayrak açılınca gider).
...
32. **Admin soru düzenleme UI (S)** — ~~backend PATCH hazır (`questionController.ts`), FE'de düzenle butonu yok~~ → salt-frontend, hızlı kazanç.
    > ⚠️ GÜNCELLEME (2026-08-17): ✅ **TAMAMLANDI, CANLIDA (çatı #87).** Kuruma özel soruya Düzenle butonu + inline form; backend PATCH `/api/questions/:id` (ADMIN + tenant-scoped IDOR) zaten hazırdı. Yalnız metin düzenlenir. CI Integration (Admin) yeşil. *(v1-E'deki kopyası 2026-08-19'da arşive alındı: `docs/arsiv/yol-haritasi-kopya-kayitlar-2026-08-19.md`; bu v1-D kaydı asıl.)*
