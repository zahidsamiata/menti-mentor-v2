...
- **9** — Algoritma kalibrasyon · **9a** ayar + **9b** motor-okuma → **🔀 backend PR #52 + çatı #114 (MERGE OLMADI; migration GEREKMEDİ).** Gösterim zaten canlıda.
...
9. **Algoritma Kalibrasyon sayfası (md.6)** · *KISMİ.* **✅ Biten (canlıda):** "Mevcut Eşleştirme Ağırlıkları" gösterim kartı (%60/%40) + `GET /api/admin/algorithm-tuner/weights` (backend #49 + çatı #102). **🔀 PR'da (MERGE OLMADI):** **9b** motor kaydedilen ağırlığı okur (backend #52, N+1 yok, regresyon testi; `scoring.ts` opsiyonel ağırlık, default=eski davranış) · **9a** tenant manuel ayar (backend #52 `PUT weights` + çatı #114 UI). **⚠️ MIGRATION GEREKMEDİ** (tenantVocabulary Json; keşif doğruladı). ⚠️ canlı etki: özel ağırlık kayıtlı tenant sıralaması aktive olur (kayıt yoksa değişmez).
