...
- **9** — Algoritma kalibrasyon · *KALAN:* ağırlık gösterimi ✅ CANLIDA; **9a** ayarlanabilirlik (migration) + **9b** (scoring dekoratif, saklanan ağırlığı okumaz) açık.
...
9. **Algoritma Kalibrasyon sayfası (md.6)** · *KISMİ.* **✅ Biten (canlıda):** "Mevcut Eşleştirme Ağırlıkları" gösterim kartı (%60/%40) + `GET /api/admin/algorithm-tuner/weights` (backend #49 + çatı #102). **🟡 KALAN (AÇIK):** **9a** ayarlanabilirlik = tenant-bazlı şema alanı (migration + canlı eşleştirme değişimi → PO onaylı) · **9b** `scoring.ts:96` hardcoded 0.6/0.4 kullanır, saklanan ağırlığı okumaz (kalibrasyon dekoratif).
