...
7. **Havuz KART + "Neden uyumlu" (KARAR 2/7 · =md.5)** · *KISMİ.* **✅ Biten (canlıda):** menti→mentör kart yönü (#39+#74); (A) mentör→menti aday kartı "neden uyumlu" gerekçe FE render (#49+#102); #7 AŞAMA 1 = değerlendirme/metrik ölü uçları bağlandı (kalite puanı + risk rozeti, backend #48 + çatı #100). Tam kayıt: `09-DURUM.md` "✅ #7 AŞAMA 1", tasarım `degerlendirme-metrik-sistemi-tasarim-2026-08-19.md`.
   - ⚠️ **madde 7-A ✅ CANLIDA — BÖLÜNDÜ (PO kararı 2026-09-08):** Havuz kartı gerekçe FE render (çatı #102) tamam. madde 7 ikiye ayrıldı; bu madde artık **yalnız (A)**'yı temsil eder. **(B) eşleşme-sonrası değerlendirme → `madde 172`** (00-KARAR-TAKIP B.1) devam ediyor.
...
30. **⚠️ Sertifika bankası canlıda eksik (5 vs 20)** — kodda 20 senaryo (`seed-certification.ts`), canlıda yalnız 5 soru (salt-okuma sayımı). Zengin banka seed edilmemiş → `seedCertification()` kontrollü çalıştırma. **Canlı DB yazımı → PO onayı ZORUNLU** (tehlikeli tam `seed.ts` değil; bu fonksiyon idempotent/silmez ama canlıda çalışır). **⚠️ GÜNCELLEME (2026-09-03, KEŞİF): kapsam netleşti** — PO kararı HARMANLA → **11 konu × 2 = 22 senaryo** (Faz 6 yapısı + isim değişkenleri + tam.md'nin 3 STK konusu geri; iki çift birleşti). ⬜ 22 senaryonun METNİ yazılmadı (içerik oturumu). Detay: `../raporlar/kesif/icerik-onkosul-kesifleri-2026-09-03.md` §A · `00-KARAR-TAKIP` md.30. **⚠️ GÜNCELLEME (2026-09-04, KEŞİF §I): sertifika kararları** — 0-3 puanlama korunuyor + puanlama rehberi + 3 oturumluk plan + kaynak doğrulanmadı notu + kriz hukuki teyit (madde 159). Detay: `../raporlar/kesif/faz5-onkosul-kesfi-2026-09-04.md` §I.
...
72. **🔵❓ Sertifika baraj "0 puan" kuralı yalnız `isRedLine`'da** — ~~[ESKİ · 2026-09-04] "tüm sorularda mı" kararı yok~~ **✅ KARARA BAĞLANDI (2026-09-04, PO): seçenek (c)** — red-line ilk-deneme `!==3` eler + diğer konularda %80-KONU eşiği (kod zaten uyguluyor: `certification.service.ts:12/50/66`). Eşik KONU bazlı (8→7), PUAN değil. ⬜ toplam eşiği kalibrasyonu (içeriği bloklamaz). detay: Bölüm F.2 (T4) + `../raporlar/kesif/faz5-onkosul-kesfi-2026-09-04.md` §I. **⚠️ EK (2026-09-04, PO): KRİTİK KONU EŞİĞİ `=== 3` → `>= 2`** — kritik 4 konuda 3 veya 2 geçer, 0/1 ELER; diğer 7 konuda ELEME YOK. Red-line'ın İKİ işlevi de korunur (garantili gelme md.149 + eleme kapısı). ~~**KOD DEĞİŞİKLİĞİ gerekir** (`certification.service.ts:66-67`).~~ **⚠️ KOD AYAĞI KODLANDI (2026-09-09, 🔀 madde 164, backend PR #71):** eşik `=== 3` → `>= 2` koda geçti (`certification.service.ts:66-73`); içerik ayağı Oturum 1'de zaten uygulanmıştı. **Karar (c) artık kodda tam yansıyor** (merge bekliyor). Detay: keşif §I-3.
73. **🟡 Güvenli sertifika seed runner ekle** (`seed-certification.ts` runner'a bağlı değil — **madde #30'u bloklar**) · *AÇIK* · detay: Bölüm F.2 (T5). **⚠️ GÜNCELLEME (2026-09-03, KEŞİF): YALNIZ sertifikayı (30) bloke eder; 147/148 kapsam DIŞI** (öğrenme yolculuğu seed'i muhafızlı+güvenli). Gerçek iş = **9 satırlık muhafız** (`seed-learning-journey.ts:534-543` desenini `seed-certification.ts`'e ekle). Kalan gerçek bloker = F.13. Kanıt: `../raporlar/kesif/icerik-onkosul-kesifleri-2026-09-03.md` §D.
