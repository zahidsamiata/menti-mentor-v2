...
> (`00-KUYRUK` 163.869 → 173.431 · `02-ILERLEME` 80.586 → 84.995 karakter) — aynı turda 8 yeni iş + 1 kart +
> 105 aile etiketi eklendi, arşivleme kazancı silindi. ⇒ **Arşivleme tek başına yetmez; asıl çözüm HEDEFLİ OKUMA**
...
|---|---|---|---:|
| 🔥 **SICAK** | her otonom turda okunur | `00-KUYRUK` · `01-KARARLAR` · `02-ILERLEME` · `03-PO-ELLE-ISLER` · `OTONOM-PROMPT.txt` · `CLAUDE.md` · `00-SIRADAKI` · `01-CEVAPSIZ` | **40.000** karakter |
| 🌡️ **ILIK** | gerektiğinde okunur | `00-KARAR-TAKIP` · `00-BELGE-HARITASI` · `00-KART-INDEKSI` · `09-DURUM` · `docs/kararlar/konu/` belgeleri | **120.000** karakter |
...
- ⚠️ `CLAUDE.md` 2026-09-23'te 34.742 karakter — 40.000'e yaklaşmasın diye buraya yalnız **tek satır atıf** girer.
- **Ölçüm her tur:** kapanış raporunda 🔥 SICAK dosyaların karakter sayısı + 1.000 karakteri aşan satır sayısı yazılır
  (`OTONOM-PROMPT.txt` § 13.5). Böylece şişme **üç hafta sonra değil, o turda** görünür.
...
- **Kapı değişken bir özelliktir** (🟡→🟢, 🔴→🟢). Kuyruğu kapıya göre dosyalara BÖLMEK her kapı değişiminde satır
  ...
  - `docs/otonom/00-SIRADAKI.md` ← `00-KUYRUK.md` baş kısmı (tanımlar) + YALNIZ 🟢 BEKLIYOR satırları
    (İş · Bitti demek; **Not kolonu alınmaz** → işe başlarken tam satır `grep -n '^| <iş-no> |'` ile kaynaktan okunur)
  - `docs/otonom/01-CEVAPSIZ.md` ← `01-KARARLAR.md`'de CEVAP satırı BOŞ kartlar (kart no · başlık · kaç işi açar ·
    kuyrukta kilitlediği 🔴 işler)
- Üretici: `scripts/otonom-turet.mjs` (`npm run otonom:turet`). **Her turun SONUNDA** yeniden üretilir, üzerine yazılır.
...
- `02-ILERLEME.md` için: son üç tur ana dosyada; öncekiler `docs/otonom/arsiv/02-ILERLEME-<YYYY-MM>.md`'ye.
