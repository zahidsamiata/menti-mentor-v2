...
**Son güncelleme:** 2026-08-02 · Bu belge SIK güncellenir — her oturum başında oku, sonunda güncelle.
...
- **Açık PR'lar:** Hiçbiri merge edilmedi. Çok sayıda iş PR'larda bekliyor.
- **En acil:** 2 IDOR güvenlik açığı + arkadaşın takılan başvurusu (gerçek kişi) + tema DISC renk kararı.

## ✅ SON YAPILANLAR (2026-08-02 oturumu)
...
### 🔴 Canlı-öncesi kritik
- 2 yeni IDOR fix (/mentors/:mentorId/candidates + /requests/:id).
...
- Sektör skoru kodu (sectorScoreResolver) — reçete var, kod stub (nötr 50 dönüyor).
- UserProfile sektör alanları migration + IndustryNode ağacı.
- Tenant plan/limit altyapısı (freemium için).
- timezone bug düzeltmesi (bookMeeting).
