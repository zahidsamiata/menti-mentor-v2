...
> **⚡ OTONOM TUR AA: OTONOM-PROMPT GÜNCELLEME + 5 UÇTAN-UCA (U) FE DÜZELTMESİ CANLIDA (2026-09-20, KOD+BELGE — çatı PR'ları, OTONOM MERGE):** Prompt bayattı (ARA KAYIT kuralı yoktu → önceki gece 2 turun emeği kayıp) → önce `OTONOM-PROMPT.txt` 7 maddeyle güncellendi (**PR #200**: ARA KAYIT · kapı politikası gevşetmesi · 6 aşama K/F/P/E/U/V · PO/ajan ayrımı · 3. mod REMOTE CONTROL · yasak bölge server.ts). Sonra uçtan-uca yolculuğu (AŞAMA U) tıkayan **5 🟢 FE düzeltmesi + 1 temizlik** işlendi, hepsi merge/CANLIDA: **U-02** toplantı linki artık kartta render ediliyor (backend gönderiyordu, 0 render — PR #201) · **U-07** onay bekleyen kullanıcı `/pending-approval`'da kendi e-postasını görüyor (token yok→query ile taşındı — PR #202) · **U-03** davet ekranında hata görünür + `{KurumAdı}` gerçek kurum adıyla dolar (eskiden sessiz+boş — PR #204) · **U-09** boş onay/bekleme panelinde yanıltıcı "işlendi" yerine doğru metin+"Davet gönder" (PR #203) · **U-11** davet süresi metni 30 güne hizalandı (kod gerçeği; PR #205) · **F-30** bayat "Sprint 14" yorumu temizlendi (PR #206). **Test:** FE suite 78/78 (11 yeni test bu tur), 6 PR'ın hepsinde CI 8/8. **⛔ TAMAMEN FE — backend/şema/DB/migration/seed DEĞİŞMEDİ · submodule pointer sabit `4528048` (backend işi yok) · `server.ts` yasak bölge DOKUNULMADI · auth guard/KVKK/matching dosyası DEĞİŞMEDİ · KIRIK TEST YOK (yalnız eklendi) · docs/gelen ELLENMEDİ · KARAR CEVAP doldurulmadı · ölü `config.invitationTokenExpiry` SİLİNMEDİ (protokol).** 🔴 sayısı 12 sabit; yeni kart açılmadı. Ayrıntı `docs/otonom/02-ILERLEME.md` (TUR AA) + `00-KUYRUK.md`. **[önceki: OTONOM TUR Z ↓]**
...
> **✅ GÜNCELLEME (2026-09-08): F.13 ✅ TEYİT EDİLDİ — banner AŞILDI.** Neon paneli PO tarafından kontrol edildi: plan **ücretsiz** · restore penceresi **6 SAAT** (maksimum ayar) · **branch oluşturma mevcut**. Migration/backfill/seed artık **YASAK DEĞİL**, ama:
> ⛔ **ZORUNLU ÖNLEM:** her migration turunda **etkilenen tablo için ÖNCE yedek tablo alınır.** 6 saatlik pencere kısa — hata 6 saat içinde fark edilmezse geri dönüş yok (özellikle hafta sonuna sarkan işlerde).
> ⬜ Ücretli plana geçiş (30 gün) PO kararı — gerçek kullanıcı verisi girmeden önce yeniden değerlendirilsin.
> ⚠️ Aşağıdaki eski uyarı **tarihsel iz** olarak kalır.
>
> ~~**⛔⛔ ACİL — DB YEDEĞİ TEYİTSİZ (2026-09-02): YENİ CANLI DB İŞLEMİ YAPMADAN ÖNCE OKU.**~~ Neon point-in-time restore aktif mi / saklama süresi NE — ~~**hiç doğrulanmadı.**~~ Bu oturumlarda geri-alınamaz canlı-DB işlemleri yapıldı (Consent backfill 08-28 · **150 satır DELETE** 08-30 · 5 FK 08-30). `MentorshipAgreement_yedek_20260830` yalnız o 150 satırı korur; ~~**DB'nin KENDİ yedeği teyitsiz.**~~ ~~⛔ **KURAL: bu teyit edilmeden migration/backfill/seed AÇILMAZ.**~~ Detay: `00-KARAR-TAKIP` F.13 · bağlı `[G1-28]` (yedekleme=en sert 🔴 bloker). **[bu tur ↓]**
>
> **2026-08-19 → 2026-09-09 arası tarih/SHA katmanları (219 satır — bu belgeden taşındı):** `docs/arsiv/09-DURUM-gecmis-katmanlar-2026-09-21.md` *(taşıma: 2026-09-21, BB turu; denetim 244+219=463)*.
>
>
...
> ⭐ **NUMARA + CANONICAL YOL EKLENDİ (2026-09-21, BB turu).** Bu satırlar numarasızdı; numarasızlık **bu turun kendi promptunu bile yanılttı** (var olan `03-PO-ELLE-ISLER.md` aranırken bulunamadı). Artık her satır kendi kartına ve canonical takip yerine işaret eder.
> ⛔ **Tam liste burada DEĞİL:** PO'nun elle yapacağı işlerin canonical kaynağı **`docs/otonom/03-PO-ELLE-ISLER.md`** (18 iş, A/B/C/D grupları). Adım adım kopyala-yapıştır karşılığı: `docs/raporlar/kesif/po-cikis-kilavuzu-2026-09-21.md`.

- **Chat uçtan uca canlı test:** menti→mentör ilk mesaj · thread · çan rozeti · okundu. → **[G8-03]** · canonical: `03-PO-ELLE-ISLER.md` **D#14**
- **Foto volume doğrulama** (Dokploy redeploy sonrası kalıcılık — `dokploy-foto-volume-talimati.md`). → **[G8-01/G8-02]** · canonical: `03-PO-ELLE-ISLER.md` **A#1** · ⛔ **ÇIKIŞ BLOKERİ (T1)**
- **Mentör paneli metriklerini canlıda gözle görme** (gerçek veri doluyor mu). → **[G8-04]** · canonical: `03-PO-ELLE-ISLER.md` **D#15**
- **Repoları PRIVATE yapma** (GitHub web ayarı; sonra Dokploy erişimini doğrula). → canonical: `03-PO-ELLE-ISLER.md` · ⚠️ bugün repo **PUBLIC**; `config.ts`'teki fallback `JWT_SECRET`/`PLATFORM_ADMIN_KEY` GitHub'dan okunabilir → **A3 (`NODE_ENV=production`) ile birlikte değerlendir.**
- **KARAR-8 ve diğer cevapsız ürün kararları** → canonical: `docs/otonom/01-KARARLAR.md` (CEVAP satırları boş).
