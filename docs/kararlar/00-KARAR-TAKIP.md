...
> TÜR: 🌡️ · SON DOĞRULAMA: ❓ içerik denetlenmedi (başlık 2026-09-23 DA turunda eklendi) · TAZELEME TETİKLEYİCİSİ: bir karar/söz geçmişi değişince (tur sonu belge senkronu)
...
> ⚠️ **ROL NOTU (2026-09-23):** Bu belge artık **KARAR VE SÖZ GEÇMİŞİDİR.** Aktif iş takibi `docs/otonom/00-KUYRUK.md`'dedir. Buradaki durum işaretleri tarihsel olabilir; güncel durum için kuyruğa bak (köprü: `docs/kararlar/00-KART-INDEKSI.md`). *(Ayrıntılı rol daraltma tablosu için aşağıya bakın — 2026-09-21.)*

**🔄 YAŞAYAN** (canonical: açık iş/karar takibi) · **Son güncelleme:** 2026-09-20 (⭐ OTONOM TUR AA — OTONOM-PROMPT #200 güncellemesi (ARA KAYIT kuralı vb. 7 madde) + **5 uçtan-uca (U) FE düzeltmesi + 1 temizlik CANLIDA**: U-02 toplantı linki #201 · U-07 bekleyen e-posta #202 · U-03 davet hata/kurum adı #204 · U-09 boş panel davet #203 · U-11 davet süresi #205 · F-30 bayat yorum #206. **NUMARA VERİLMEDİ (aday):** hepsi `00-KUYRUK` AŞAMA U/F kalemleri, statü orada BITTI; ayrıntı `02-ILERLEME.md` TUR AA. **TAMAMEN FE** — backend/şema/DB/migration/seed DEĞİŞMEDİ, pointer sabit `4528048`, `server.ts` yasak bölge/auth/KVKK/matching DOKUNULMADI, yeni KARAR açılmadı (🔴 sabit 12), CEVAP doldurulmadı, ölü `config.invitationTokenExpiry` SİLİNMEDİ) · geçmiş: bkz. `## GEÇMİŞ` §son-guncelleme
> ⭐ **ROL DARALTMASI (2026-09-21) — bu belge artık İŞ TAKİBİ YAPMAZ.**
> ⛔ **EMEKLİ EDİLMEDİ, HİÇBİR SATIR SİLİNMEDİ.** Ölçüm: son 14 günde **45 commit** — depodaki 2. en aktif belge; ölmedi. Sorun, motorun hem buraya hem kuyruğa yazması → aynı iş iki yerde, çift bayatlama.
>
> | konu | artık nerede |
> |---|---|
> | Açık iş · yarım iş · durum (BEKLIYOR/BITTI/PR-ACIK) | ⛔ **`docs/otonom/00-KUYRUK.md`** — TEK canonical |
> | PO'nun elle yapacağı işler (Dokploy/SMTP/Neon/env) | `docs/otonom/03-PO-ELLE-ISLER.md` |
...
> | **ÖLÜ KOD / TERK NİYETİ** (silme protokolü izi) | ✅ **BU BELGE** (iş hâline gelince kuyruğa satır açılır) |
>
> **Yeni yazım kuralı:** (1) Buraya **durum güncellemesi için gelinmez** — ✅/🟡 kuyruğa yazılır, buraya yalnız o işi doğuran **KARAR** ve **SÖZ**. (2) Madde numaraları **SİLİNMEZ/TAŞINMAZ**; statüsü kuyruğa geçmiş maddenin durum hücresine `→ statü: 00-KUYRUK <satır>` **pointer'ı** yazılır. (3) ⛔ Daralma **yeni yazımın kapsamıdır**; eski içerik yerinde kalır.
...
> ~~[ESKİ · 2026-08-29] **⭐ G1-23 → 🗑️ GEÇERSİZ (kanıtlı):** logoUrl 3 yazma yolunun üçünde de admin+kendi-tenant guard'ı VAR (`selfServeController.ts:~388`, `tenantController` `requirePlatformAdmin`). "Guard yok" iddiası eskimiş.~~
...
> - **✅ EKRAN KARARI:** üç soru kayıt sonuna, arketip sonrası; S2 NÖTR metin. **⚠️ GÜNCELLEME (2026-09-04, KURAL 12): TERSİNE ÇEVRİLDİ** — arketip §4 (2026-09-03): üç soru arketip kartından ÖNCE, kart SONDA. Detay: tasarım §10.2 (damgalı) + madde 141.
...
> **⚡ GÜNCELLEME (2026-09-02, 2) — G1 ÇAKIŞMA DÜZELTMESİ + KAYNAK HİYERARŞİSİ (yalnız-belge, PR #150 dalına devam — MERGE ETME):** Önceki G1 turunun iki numara çakışması PO kararıyla çözüldü. **İŞ 1 — G1-19 (çift-uygulama):** 11 G2 kartı tarandı, hiçbiri kapsamıyor (KAPSAM DIŞI) → çift-uygulama zaten tasarım §9.5 + KALEM §16-md.8'de homed; KARAR-TAKIP satır 79'daki yanlış "G1-19" etiketi G9-03'le damgalandı (numara VERİLMEDİ). **İŞ 2 — G1-23 (⭐ 21. HAYALET):** "🗑️ geçersiz" logoUrl **sahiplik guard'ı** içindi (doğru) ama kart konusu **XSS** (kodda AÇIK) → KISMİ KANITLA TAM KAPATMA. G1-23 kartı **❓→⬜ AÇIK**'a döndü; KARAR-TAKIP satır 62 damgalandı; sahiplik tespiti korundu. **İŞ 3:** G1 kart dosyası **📸→🔄 YAŞAYAN** (yalnız G1; diğer kart dosyaları kendi turlarında). **İŞ 4:** kök CLAUDE.md'ye **KURAL 15 ADAYI — kaynak hiyerarşisi** (kart=ayrıntı+kanıt · KARAR-TAKIP=özet · çelişkide KART kazanır). **İŞ 6 — TÜM 🗑️ TARAMASI (yalnız tespit, G1-23 dışı hiçbir durum değişmedi):** iki-dilli tara (geçersiz↔invalid) — geçersiz-etiketli tracked kalemler: G2-01..05 (PO-note, DISC→Big Five), G9 5 belge-hijyen kartı, G10-376 (dosya yok), G10-63 (ikiz-iddia çürük), T8/md.76 (çelişki çözüldü), md.129 (belge bayat), G1-23. ⭐ **G1-23 TEK partial-closure hayaleti** (🔴); geri kalanların hepsi 🟢 TAM (konu tümden geçersiz / iddia çürük / belge-hijyen) — kanıt: PO-note'lar + kod-teyit (dosya yok / iddia çürütüldü). Kod/DB DEĞİŞMEDİ. **MERGE EDİLMEDİ.**
>
> **⚠️ NOT (sonraki grup turları):** G2-G11 kart dosyaları HENÜZ 📸; kendi çapraz-doğrulama turlarında 🔄'ye çevrilecek + KURAL 15 kısa notu eklenecek (bu turda yalnız G1'e dokunuldu).
...
| S1 | 6 arşiv teyidi yapılacak | 2026-08-14 | ~~⬜~~ ✅ TUTULDU · ⚠️ **PO KARARI (2026-09-08): ✅ TUTULDU — HİÇBİRİ SİLİNMİYOR.** Altı belge okundu: beşi `docs/raporlar/kesif/` altında `📸 DONDURULMUŞ` (2026-08-02, 45-132 satır), altıncısı `docs/devir/` **dizini** (8 dosya, 1773 satır). ⭐ **"Silinirse ne kaybolur" testi** — her biri başka yerde olmayan bilgi taşıyor: `hayalet-backend` (dosya:satır kanıtlı ölü-kod envanteri) · `kapasite-analizi` (tek somut canlı-öncesi ölçeklenme listesi) · `katilim-modeli` (hayalet-mod boşluk analizi) · `mentor-karti` (PO'nun 6 kesinleşen kart kararı) · `tema-durum` (landing maliyet tahmini, 256 nokta / ~10-13 sa) · `devir/` (oturum tarihçesi). ⚠️ **"ARŞİV" KELİMESİ YANILTICIYDI** — belgeler `docs/arsiv/`'de değil, keşif klasöründe dondurulmuş halde. Söz "6 arşiv teyidi" dendiği için herkes temizlik işi sandı; oysa **geçerlilik kontrolüydü.** (Bkz. aynı turda yazılan KURAL 11 üçüncü katmanı.) · geçmiş: bkz. GEÇMİŞ §S1 | — (belge-hijyen) |
...
| S29 | **Davet-APPROVED backend PR #67 merge olunca** çatı submodule pointer'ı backend `main` HEAD'e **re-bump** et (S22-S28 deseni). ⭐ Bu bump'tan SONRA çatı e2e (PR #148) YEŞİLE döner (davetli /onboarding'e ulaşır). | 2026-09-01 | ✅ **YAPILDI (2026-09-02):** backend #67 (`1176a6d`) + çatı #148 (`3c26a5d`) merged. Pointer `882ce97 → 1176a6d` re-bump (`chore/pointer-rebump-s29`, PR #149). İleri sarım teyitli. ⭐ **e2e YEŞİLE DÖNDÜ** — CI run `33605765593` `e2e-browser` **2 passed** (menti + mentör onboarding açılır). | login-PENDING / backend PR #67 / çatı #148 / pointer #149 |
| S30 | ⭐ **Faz 5 promptu yazılırken tasarım belgesi §16 KALEM LİSTESİ AÇIKÇA OKUNACAK** — özellikle **md.8 kalite çarpanı çift-uygulama** (PO kararı 2026-09-02: numara VERİLMEDİ, `konu/degerlendirme-sistemi-tasarim-2026-08-27.md` §16'da kalır; gerçek soru `(base+bonus)×qm` bilinçli tasarım mı). ⚠️ Gerekçe: numarasız kalem takipten düşer; "belgede duruyordu ama kimse bakmadı" olmasın. | 2026-09-02 | ⬜ BEKLİYOR | Faz 5 / tasarım §16-md.8 (çift-uygulama; eski yanlış etiket "G1-19" düzeltildi) |
| S31 | `outcome` alanı bugün nereye gidiyor — yalnız tamamlama takibi mi, yoksa kişilik profiline mi işleniyor? **Profile işleniyorsa KESİLMELİ** (madde **145** gereği: öğrenme yolculuğu cevapları kişilik profilini beslemez). Kaynak: `../raporlar/icerik/kod-kalemleri-2026-09-03.md` Kalem M5 (kod kalemi DEĞİL, keşif). | 2026-09-03 | ✅ **YAPILDI (2026-09-03, KEŞİF):** outcome kullanıcı seçimi HİÇBİR YERE yazılmıyor → profile İŞLENMİYOR (kanıt: `learningJourney.service.ts` 553/553 + controller 107/107, 7 terim/2 dil = 0 sonuç). madde 145 = koruma işi. Detay: `../raporlar/kesif/icerik-onkosul-kesifleri-2026-09-03.md` §B | 145 |
| S32 | Backend PR #68 merge sonrası çatı pointer re-bump (S22-S29 deseni). | 2026-09-04 | ✅ **YAPILDI (2026-09-04):** `10bba42 → df40d03` (Merge PR #68), ileri sarım teyitli, çatı commit `c00787e`, PR #157'ye eklendi. | 145 / backend #68 / pointer |
| S33 | Faz 5 promptu yazılırken **6 KARAR NOKTASI** (keşif §G) PO'ya sorulacak — özellikle "skor hangi alana yazılacak (`oceanO..N` mi `discD..C` mi)" ve "canlı `matching.ts` mi uykudaki `rankMentorsForMenti` mi". **⚠️ GÜNCELLEME (2026-09-08): 6 → 7 KARAR NOKTASI.** Yeni 7. karar: *Üç DISC giriş yolundan hangisi `UserProfile.discD..C`'yi dolduracak — hepsi mi, yalnız biri mi; diğerleri o alana bağlanacak mı?* Bu karar verilmeden Faz 5 (g) ve (k) tanımlanamaz. **Mevcut 6 kararın KANIT DURUMU (keşif §E):** karar 1 ✅ (adapter'ı tek yer çağırıyor) · karar 2 ✅ (alan sahipliği net) · karar 3 🟡 (iki motor tablosu var; backfill boyutu ❓ + FE DTO eforu eksik) · karar 4 ✅ (`SjtResponse` yok → tek seçenek backfill) · karar 5 ✅ (`forRole` okuyan runtime kod 0 → nullable neredeyse bedava) · karar 6 ✅ (migration yok, 8 kod noktası). ⭐ **İKİ `rankMentorsForMenti` VAR** — `matching.ts:351` (DISC, CANLI, `matchingController:107`, testli) ve `scoring.service.ts:165` (arketip, uykuda, `sjtScoringController:133`, FE'siz, **0 testi**). Karar 3 tam olarak bu ikilik hakkındadır. Detay: `faz5-veri-akisi-kesfi-2026-09-08.md` §C/§F. · **✅ 7. KARARIN CEVABI (2026-09-08, KEŞİF):** *"Üç DISC giriş yolundan hangisi `UserProfile.discD..C`'yi dolduracak?"* → **YOL 3 (zorunlu onboarding quiz)**, çünkü canlı akış odur (madde 161). Yol 1 ve 2 opsiyonel. Kanıt: madde 161/162. · **⚠️ KARAR 3'E ETKİ:** *"canlı `matching.ts`'i mi çevirelim, uykudaki motoru mu bağlayalım?"* sorusu artık yeni bir gerçekle karşı karşıya — **uyku motoru bugünkü haliyle BAĞLANAMAZ.** Ölçek uyuşmazlığı düzeltilmeden bağlanırsa her kullanıcı M1/m1 alır. Karar 3 verilmeden önce ölçek kalemi çözülmeli. | 2026-09-04 | ⬜ BEKLİYOR | 101 / Faz 5 · `faz5-onkosul-kesfi-2026-09-04.md` §G |
...
| S36 | ⭐ Çatı submodule pointer'ı backend main'e re-bump edildi mi (S22-S29 deseni) | 2026-09-08 | ✅ **YAPILDI (2026-09-08):** pointer `5bd1656 → b1a007f` (backend #69 merge commit); ileri sarım teyitli (`5bd1656`, `b1a007f`'nin atası); çatı dalı `docs/gun-kapanisi-2026-09-08` | 73 |
| S37 | ⭐ **Yedek tablo `CertificationOption_yedek_20260909` DROP edilecek** (`DROP TABLE`). **Tetik:** sertifika akışı regresyonsuz görülünce (S26 deseni). ⚠️ Tablo `schema.prisma`'da YOK; ileride `migrate dev`/`db push` onu fazlalık görüp DROP etmek isteyebilir — o zaman gelmeden bilinçli silinmeli. | 2026-09-09 | ⬜ BEKLİYOR | 163 |
...
> ## ⭐ DURUM KODU ↔ ENGEL İŞARETİ AYRIMI (PO kararı, 2026-09-08)
>
> **Durum kodu** (işin hazırlık düzeyi): `✅` `🟡` `🔀` `⬜` `🔵` `🔴`
> **Engel işareti** (ne bekliyor): `❓` karar/keşif · `⛔` bloker
>
> · **Bir durum + bir engel GEÇERLİDİR:**
>   `🔵❓` = tasarım hazır, PO kararı bekliyor
>   `🔴❓` = kritik + karar bekliyor
>   `🔵⛔` = tasarım hazır, bloker var (ör. `🔵 ⛔MOTOR`)
> · **İki DURUM kodu GEÇERSİZDİR** (`🔴/🟡` gibi) — biri seçilmeli ya da **satır bölünmeli.**
>
> ⚠️ **Bu kural mevcut kullanımı MEŞRULAŞTIRIR, yenilik getirmez:** `⛔` zaten madde 138-140'ta (`🔵 ⛔MOTOR`) ve 162'de (`⬜ ⛔BLOKE`) böyle kullanılıyordu — sözlükte tanımlı değildi.
>
> ⚠️ **ÖNCEKİ TURUN "ÇİFT KOD" NOTLARI GERİ ALINDI:** 2026-09-08 kod temizliği turunda 8 maddeye "çift kod = belirsizlik" notu düşülmüştü. Bu kuralla **7'si GEÇERLİ** hale geldi: 33 (`🔴❓`) · 36 · 86 · 101 · 103 · sektör-havuzu · DISC-derinleşme (altısı `🔵❓`). **Yalnız Y7 (`🔴/🟡`) geçersiz kaldı.**
>
> ⚠️ **ÇİFT KOD KURALI (yukarıda, 2026-09-08 önceki tur) GÜNCELLENDİ:** "iki durum kodu belirsizliktir" hükmü DOĞRU kalır, ama `🔵❓`/`🔴❓`/`🔵⛔` **iki durum kodu DEĞİL** — bir durum + bir engeldir, GEÇERLİDİR.

> ## NUMARA BOŞLUĞU KURALI (2026-09-08)
> Numara boşlukları **bilinçlidir.** Boş numara **YENİDEN KULLANILMAZ**; boşluk kendi hikâyesini anlatır. Numarayı yeniden kullanmak eski atıfları sessizce yanlış hedefe bağlar.

> ## KURAL 11'E EK — SÖZ İÇERİĞİ (2026-09-08)
> Söz verilirken **NEYİN** sözü olduğu **tek cümleyle** yazılır.
...
| 13 | Soru cevap-tipi seçimi (şıklı/açık-uçlu) | ~~✅ İŞLEME AL~~ 🔵 (PO 2026-08-27) | canlandı (G3-13) | ⚠️ **KOD DÜZELTMESİ (2026-09-08):** eski durum `✅ İŞLEME AL` yanıltıcıydı — ✅ "yapıldı" demektir, oysa bu madde AÇIK. PO 2026-08-27'de işleme almaya karar verdi ama iş yapılmadı. Doğru kod: 🔵 (tasarım-hazır). · **⚠️ GÜNCELLEME (2026-08-27): PO kararıyla ⏸️→✅ İŞLEME AL** — STK şıklı-soru + anket isteği (G3-04) `answerType` şema alanını **zorunlu kılıyor (migration)**; artık ertelenmiş değil, sıraya girecek (öncelik ayrı tur). · Kapsam belirsiz (tipler/validation/skoring) → **PO netleştir**; sonra şema alanı = migration · geçmiş: bkz. GEÇMİŞ §md.13 · **⚠️ GÜNCELLEME (2026-08-26, KEŞİF):** kurum-özel `Question`/`UserResponse` yalnız Likert 1-5; cevap-tipi alanı YOK → yeni tip = **migration** (kanıt döküm §6). Şıklı altyapı SJT'de var ama STK_CUSTOM kullanmıyor. Detay: `../raporlar/icerik/tam-soru-dokumu-2026-08-26.md`. | tam-envanter A4; `schema.prisma` alan yok | **Evet** | M |
| 30 | Sertifika bankası 5→20 canlı seed | 🔴 | içerik-eksik | `seedCertification()` kontrollü çalıştır (idempotent) → **canlı DB yazımı, PO onayı ZORUNLU** · **⚠️ GÜNCELLEME (2026-08-26, KEŞİF):** kod 20 senaryo (4 red-line) teyitli; **güvenli seed runner YOK (madde 73 bloke)**; canlı sayısı ⏳ TEYİT GEREK (DB). Detay: `tam-soru-dokumu-2026-08-26.md`. · **⚠️ GÜNCELLEME (2026-09-04, KEŞİF §I): sertifika kararları netleşti** — **0-3 puanlama KORUNUYOR** (kod zaten 0-3, T2) + puanlama rehberi (3/2/1/0 tanımları, red-line'da 0 daha dar) + sahne seçim kriteri (ayrışma>STK>çakışmazlık) + **3 oturumluk plan** (O1: 4 kritik/8 senaryo · O2: 2 birleşen+aktif dinleme/6 · ~~O3: 5 konu/10~~ → ⚠️ düzeltme (2026-09-08, KALEM 2): O3: 4 konu / 8 senaryo / 32 şık) + **kaynak DOĞRULANMADI** notu (CIMER/NCSU — internet yok) + 11 konu birleştirme kaydı. Detay: `../raporlar/kesif/faz5-onkosul-kesfi-2026-09-04.md` §I. · **⚠️ EK (2026-09-04, PO): kritik konularda 2 GEÇER** (eşik `>=2`) → 32 kritik-konu şıkkı yazılırken **1↔2 çizgisi (prensip UYGULANDI mı = geçme/kalma çizgisi)** her kritik senaryoda açıkça görülmeli. Detay: keşif §I-3. · **⚠️ GÜNCELLEME (2026-09-08): ⭐ İÇERİK SERİSİ TAMAMLANDI.** ~~[ESKİ] 22 senaryonun METNİ YAZILMADI (içerik oturumu)~~ → **11 konu × 2 varyant = 22 senaryo · 88 şık YAZILDI ve kaydedildi.** Üç belge: `../raporlar/icerik/sertifika-oturum1-4-kritik-konu-2026-09-08.md` (4 kritik konu / 32 şık / KALEM 1-10) · `../raporlar/icerik/sertifika-oturum2-3-konu-2026-09-08.md` (3 konu / 24 şık / KALEM 11-16) · `../raporlar/icerik/sertifika-oturum3-4-konu-2026-09-08.md` (4 konu / 32 şık / KALEM 17-19 · seri kapanış tablosu). Kaynak dağılımı: faz6 15 · `tam.md` 5 · yeni 2 sahne. · **⚠️ SEED ÖNCESİ ZORUNLU SIRA (2026-09-08):** **F.13 (Neon yedeği) → madde 163 (iç-not alanı, MIGRATION) → madde 164 (eşik `>= 2` + test) → madde 30 (seed).** · geçmiş: bkz. `## GEÇMİŞ` §md.30 · ⚠️ **SON ÖNCÜL 🔀 (2026-09-09): madde 164 KODLANDI** (backend PR #71; eşik `>= 2` koda geçti + test düzeltildi/eklendi). Merge olunca **kod zinciri TAMAM.** ⚠️ Ama seed hâlâ **madde 159** (kriz senaryolarının hukuki teyidi) ve **KALEM 8** (kriz geri bildiriminde somut destek kaynağı adı) blokerlerini bekliyor — ikisi de PO işi, kod değil. | tam-envanter A5; canlı ~5, kod 20 | **Evet (seed)** | S |
...
| 33 | SJT belge-kod (3 vs 4) + seed↔canlı (32 vs 20) kalan | 🔴 ❓ | içerik-eksik | ~~⚠️ **ÇİFT KOD (2026-09-08):** bu madde iki durum kodu taşıyor (belirsizlik). Tek koda indirilmesi PO kararı — bkz. kod sözlüğü ÇİFT KOD KURALI.~~ ✅ **GEÇERLİ (2026-09-08, PO):** `🔴❓` = durum+engel kombinasyonu (kritik + karar bekliyor), belirsizlik değil — bkz. kod sözlüğü DURUM↔ENGEL AYRIMI. · (a) seed↔canlı: re-seed mi trim mi (canlı DB, PO); (b) SJT 3→4 içerik genişletme (PO) · **⚠️ GÜNCELLEME (2026-08-26, KEŞİF):** KOD gerçeği kesin — **SJT=3** (belge "4" yanlış), **DISC=32** (belge "20"=silinmiş `seed-questions.ts`, tek kaynak artık `seed.ts`). ⚠️ SJT **OCEAN** ölçer (DISC değil) ve **canlı eşleştirmede OKUNMUYOR** (madde 101). Canlı sayılar ⏳ TEYİT GEREK. | tam-envanter C4; `03-psikometri:47` "4", kod 3 | **Evet (a)** | S |
...
| 36 | **(2b)** Onaylanmış (aktif) kullanıcıyı sistemden çıkarma (KARAR 3) | 🔵 ❓ | tasarım-hazır + önce-keşif | ~~⚠️ **ÇİFT KOD (2026-09-08):** bu madde iki durum kodu taşıyor (belirsizlik). Tek koda indirilmesi PO kararı — bkz. kod sözlüğü ÇİFT KOD KURALI.~~ ✅ **GEÇERLİ (2026-09-08, PO):** `🔵❓` = durum+engel kombinasyonu (tasarım hazır + karar/keşif bekliyor), belirsizlik değil — bkz. kod sözlüğü DURUM↔ENGEL AYRIMI. · **ÖNCE git'ten doğrula** (isActive=false/demote kodda var mı?), eksikse yap | `11-tasarim-kararlari` KARAR 3; `10-yol:md.36` | ❓ (keşif sonrası) | M |
...
| 147 | Menti öğrenme yolculuğu — 5 aşama, mentör yolculuğuyla aynı altyapı, sınavsız | 🔵 | içerik-seed | LearningStage'e menti 5 aşama seed. ~~[ESKİ · 2026-09-03] ⛔ md.73 seed runner + F.13 yedek bloke~~ **⚠️ GÜNCELLEME (2026-09-03, KEŞİF): md.73 kapsam DIŞI** — `seed-learning-journey.ts` güvenli+idempotent, doğrudan-çalıştırma muhafızı zaten VAR (`:534-543`), deleteMany yok. Kalan bloker yalnız **F.13** (canlı DB, PO onayı). Kanıt: keşif §C/§D | menti §3/§11-1 | Evet (seed) | M |
| 148 | ⭐ Öğrenme yolculuğu içeriği YENİ TASARIMLA DEĞİŞTİRİLSİN — canlı Mentör 7+Menti 6 → yeni Mentör 8+Menti 5 (yeni tasarım canlının YERİNE geçer, geçiş planı gerekmez) | 🔵 | içerik-seed | Canlı **`seed-learning-journey.ts`** içeriğini yeni 8+5 ile değiştir (**DISC'e DOKUNMAZ**). ~~[ESKİ · 2026-09-03] (⛔ md.73+F.13 bloke)~~ **⚠️ GÜNCELLEME (2026-09-03, KEŞİF):** (1) md.73 kapsam DIŞI, kalan bloker **F.13** · (2) ⭐ **PASİFLEŞTİRME kapsama EKLENDİ** — salt re-seed menti 6→5'te eski `seed-ls-menti-6`'yı **aktif öksüz** bırakır; `updateMany{isActive:false}` deseni eklenmeli (mentör 7→8 yeni id create, sorunsuz). Kanıt: keşif §C/§D | kod-kalemleri **Madde 2** + `seed-learning-journey.ts:7` | Evet (seed) | L |
...
| 161 | Canlıda hangi DISC yolu kullanılıyor — FE akışı keşfi + canlı doluluk sorgusu (❓ DB, PO onayı) | ~~⬜ AÇIK~~ ✅ CEVAPLANDI | KEŞİF | ⚠️ **KOD NOTU (2026-09-08):** ✅ burada "keşif sorusu cevaplandı" anlamındadır (madde "yapıldı" DEĞİL); doğan iş madde 162'de takip edilir. · ⚠️ 162 ve 165'in kapsamı buna bağlı · **✅ CEVAPLANDI (2026-09-08, KEŞİF): CANLI AKIŞ YOL 3'Ü KULLANIYOR.** Kayıt sonrası zorunlu onboarding wizard'ının 1. adımı `submitDiscAnswers` → `POST /api/users/disc/submit` (`onboardingController.ts` `submitDiscTest`) — bu yol **yalnız** `User.discType`/`discVector`/`discResultCard` yazar, `recalcDiscVector` ÇAĞRILMAZ (KAPSAM: `backend/src`, harf duyarsız, `recalcDiscVector` → yalnız 2 çağrı: `questionController.ts:314` ve `:378`, ikisi de Yol 1). · **Yol 1** (`/disc-test` dashboard sayfası, `useDiscTest.ts:148`) CANLI ama **OPSİYONEL** — kullanıcı isterse girer. · **Yol 2** (adaptif test widget'ı, `DailyQuestionWidget.tsx`) CANLI ama **OPSİYONEL** — "Daha sonra" ile atlanır. · ⭐ **Onboarding Step 0** (`completeProfile`) `UserProfile` satırını OLUŞTURUYOR ama yalnız `skillTags`/`goalTags`/`industryCode`/`yearsExp` yazıyor — `discD..C` DEĞİL. → Satır VAR, DISC alanları `@default(0)`. · **SONUÇ:** onboarding'den geçmiş ama `/disc-test`'e hiç girmemiş kullanıcıda `UserProfile.discD..C` **tam sıfır** kalır. `computeAndStoreProfile` çağrılırsa throw değil, **tümü-50** üretir (`scoring.service.ts:90` throw yalnız satır YOKSA). ⚠️ `recalcDiscVector` **asla 0 yazmaz** — hiç cevap yoksa bile 0.25 prior koyup 1.0'a normalize eder (`:124-137`). Yani `discD..C = 0` YALNIZCA o fonksiyon hiç çağrılmamışsa oluşur. · ⬜ **AÇIK KALAN:** canlıda kaç kullanıcının `discD..C`'si hâlâ 0 → ❓ TEYİT GEREK (DB sorgusu, PO onayı). | `../raporlar/kesif/faz5-veri-akisi-kesfi-2026-09-08.md` §E | Hayır | M |
| 162 | Üç DISC yolunun birleştirilmesi — `UserProfile.discD..C`'yi hangi yol dolduracak + eksik yolları bağlama | ⬜ AÇIK | KOD+KARAR | ⚠️ S33'ün 7. kararı; Faz 5 (g)/(k) bunsuz tanımlanamaz · **⚠️ KAPSAM NETLEŞTİ (2026-09-08, KEŞİF):** Zorunlu akış Yol 3 olduğu için `UserProfile.discD..C`'yi **Yol 3 doldurmalı** — ~~`submitDiscTest` içine `recalcDiscVector` çağrısı ya da eşdeğeri~~. Migration GEREKMİYOR, alanlar zaten var (`schema.prisma:988-991`, `Float @default(0)`). Yol 1 ve Yol 2 opsiyonel; onlar da doldurursa iyi olur ama kritik olan Yol 3. Bu, **S33'ün 7. kararının cevabıdır.** · **⚠️ KAPSAM DÜZELTMESİ (2026-09-08, KEŞİF): ÖNCEKİ TANIM UYGULANAMAZ.** ~~[ESKİ] `submitDiscTest` içine `recalcDiscVector` çağrısı~~ → **`recalcDiscVector` verisini `UserResponse` tablosundan (Likert) okuyor; onboarding quiz'i A/B/C/D zorunlu-seçimdir ve `UserResponse` YAZMAZ** (`:449-501`). Çağrı eklenirse fonksiyon okuyacak veri bulamaz → vektörü düz 0.25'e ezer ya da matching skorlarını kaydırır. **Bu, "bugün çalışanı bozma" riskidir.** · ⭐ **GERÇEK ÇÖZÜM — `buildDiscVector` ortak yardımcısı.** Üç DISC yazıcısı (`recalcDiscVector` · `adaptiveTestEngine` · `submitDiscTest`) tek bir `buildDiscVector(...)` yardımcısından geçirilsin; yardımcı `confidence` alanını da üretsin. Bu **hem** onboarding vektörünün `scoring.ts:72` kapısından geçmesini sağlar **hem** üç yazıcıyı hizalar. Boyut: **S**. · **⚠️ ÖLÇEK ENDİŞESİ ÇÜRÜDÜ (canlı taraf):** üç yazıcının ÜÇÜ DE 0-1 (toplam 1.0) yazıyor — `discVectorService.ts:131-143` · `adaptiveTestEngine.ts:86-98,240` · `onboardingController.ts:232-237,478`. Okuyucu `computeVectorDiscScore` (`scoring.ts:53-61`) tam bunu bekliyor; `discLetters` eşikleri (0.25 orta nokta, `discLetters.ts:35,75`) de aynı ölçekte. **Canlı DISC ölçeği TUTARLI.** · ⚠️ **GERÇEK SORUN `confidence` EKSİKLİĞİ:** `calculateDiscResult` (`onboardingController.ts:201-204`) dönüş tipinde `confidence` YOK. `scoring.ts:72` kapısı `mentiVector.confidence > 0` istiyor → `undefined > 0` = false → **onboarding kullanıcılarının ince vektörü atlanıp tek-harf `DISC_COMPATIBILITY` matris skoruna düşülüyor.** Skor geçerli çıkıyor, sadece kaba. Sessiz kayıp. · ⚠️ **ÖLÇEK BLOKAJI DEVAM EDİYOR ama BAŞKA MOTORDA.** Bu maddenin `⛔ BLOKE` notu **OCEAN motoru** içindir (F.14 ölçek kalemi) ve geçerlidir. Canlı motor ile karıştırılmamalı: **canlı ✅ tutarlı, OCEAN ✗ bozuk.** · ⬜ **AÇIK KALAN:** onboarding kullanıcılarının `UserResponse` satırı var mı → ❓ TEYİT GEREK (DB sorgusu, PO onayı). Bugün 6 test kullanıcısı olduğu için pratik etkisi düşük; gerçek kullanıcı gelmeden sorulmalı. Detay: `../raporlar/kesif/` — bu keşif henüz belgeye yazılmadı, kaynak sohbet oturumu 2026-09-08. · ⛔ **BLOKE — ÖLÇEK UYUŞMAZLIĞI ÖNCE ÇÖZÜLMELİ.** Yol 3'e `recalcDiscVector` bağlansa BİLE arketip yine fallback M1/m1 çıkar (bkz. F.14 ölçek kalemi). **SIRA: ölçek düzeltmesi → madde 162 → Faz 5.** | `../raporlar/kesif/faz5-veri-akisi-kesfi-2026-09-08.md` §E | Hayır (alanlar mevcut) | L |
...
| 167 | ⭐ Öğrenme yolculuğuna GİZLİLİK ve BİTİRME aşamaları eklenecek — mentör yolculuğu 8 → 10 aşama. Gizlilik RED-LINE olduğu için öğretilmemiş konuda eleme "öğretimin son tekrarı" felsefesini bozuyor. Felsefeye istisna KONMAYACAK. | ⬜ AÇIK | İÇERİK (yazım turu) | Sertifikada SINANIYOR ama öğrenme yolculuğunda karşılığı YOK (KALEM 19) | `../raporlar/icerik/sertifika-oturum3-4-konu-2026-09-08.md` KALEM 19 | Hayır | — |
...
> ⚠️ Bu bölümdeki `⏸️` kodu 2026-09-08'de kod sözlüğüne eklendi: **v2'ye / bilinçli ERTELENDİ** (v1 kapsamı dışı,
> bilinçli karar). Aynı `⏸️` B.2'de K6/K3'te ve C bölümünde U1/U2'de de kullanılıyor (durum kodu, değiştirilmedi).
...
| Y7 | Platform **büyüme metrikleri** (ivme, aktif/pasif oran) + platform **ayar UI** + 3. seviye kullanıcı drill | ~~🔴/🟡~~ 🗑️ BÖLÜNDÜ | geçmiş: bkz. GEÇMİŞ §Y7 · ⚠️ **PO KARARI (2026-09-08): BÖLÜNDÜ.** Üç ayrı iş — farklı büyüklük, farklı bağımlılık: platform büyüme metrikleri (veri işi) · platform ayar UI (arayüz) · 3. seviye kullanıcı drill (ikisinin üstüne gelen derinleştirme). Tek satırda `🔴/🟡` çelişkisi çözülemiyordu. **madde 7 ile aynı gerekçe.** ⚠️ **`Y7` numarası YENİDEN KULLANILMAZ** (numara boşluğu kuralı). Üç yeni parça: **Y9 · Y10 · Y11** — ⚠️ **Y8 DOLU** (kulüp CANLI NİYET, 2026-09-01, `00-KARAR-TAKIP:438`); boşluk kuralı gereği Y8 atlandı (prompt örneği "Y8·Y9·Y10" bu yüzden düzeltildi). Bu satır tarihsel iz. · Düşük öncelik (platform admin = PO); canlı-sonrası | denetim B.4/3,4,12,18 |
...
| **103** | **Psikometrik gerekçe BELGELENMEMİŞ** — DISC uyum matrisi (16 değer), tiebreak D>I>S>C, %60/%40 ağırlık, anti-match, harf eşikleri (0.25/0.75) elle sabit; ampirik/teorik kaynak YOK (`discLetters.ts:23` "başlangıç değerleri, kalibre edilecek"). Uzman görüşü/pilot-veri kalibrasyonu iş kalemi mi? PO uyum tablosuna `eslesme-uyum-po-inceleme` ile karar verebilir. | teknik-borç/kalite (KVKK-otomatik-karar akrabası) | döküm §7; `scoring.ts:44-49`, `discLetters.ts` | 🔵❓ PO · ~~⚠️ **ÇİFT KOD (2026-09-08):** iki durum kodu (belirsizlik); tek koda indirme PO kararı — bkz. kod sözlüğü ÇİFT KOD KURALI~~ ✅ **GEÇERLİ (2026-09-08, PO):** `🔵❓` = durum+engel kombinasyonu (tasarım hazır + karar bekliyor), belirsizlik değil — bkz. kod sözlüğü DURUM↔ENGEL AYRIMI · ⚠️ GÜNCELLEME (2026-08-28): tasarım belgesinde ele alındı (gerekçeler B9/B15) → `konu/degerlendirme-sistemi-tasarim-2026-08-27.md` |
...
| 109 | Menti P1 DISC "özgüven aşısı" sunumu | ~~⬜ AÇIK (PO önceliklendirmedi)~~ 🗑️ GEÇERSİZ | ⬜ | T2-D(menti:80) | Menti-yönü özel "değerlisin" sunumu | Menti-yönü özel sunum yapılmadı; DURUŞ SEBEBİ YOK · ⚠️ **KAYNAK İZİ (2026-09-08):** F.6'da `b141738` (2026-08-26, "numara adaylarını numaralandır F.6: 104-124 — PO talimatı") ile numaralandı; niyet kaynağı bilanço `T2-D(menti:80)`. ⬜ Duruş sebebi hâlâ belgelenmemiş; ~~**PO kararı gerekiyor: madde hâlâ geçerli mi, yoksa 🗑️ mı?**~~ · ⚠️ **PO KARARI (2026-09-08): 🗑️ GEÇERSİZ.** "Menti P1 DISC özgüven aşısı sunumu" **DISC dönemi fikriydi**; artık DISC sonucu gösterilmiyor, **arketip kartı** gösteriliyor. ⭐ **Fikir ÖLMEDİ, EVRİLDİ** → **madde 138-140** (arketip kartı: 8 kart + "şimdilik" dili) aynı işi yapıyor. Bu satır tarihsel iz olarak kalır. |
...
| 121 | `backend/.env.backup-anaDB` sil (env geçişi bitince) (= G8-05, bkz. bilanco/kararlar/G8-altyapi-po-manuel.md) | ⬜ AÇIK (PO önceliklendirmedi) | ⬜ | T2-B(depo:25) | Env geçişi bitince yedek dosyayı temizle | Dosya HÂLÂ VAR (`ls` teyit) |
| 122 | `PROJECT_STATUS.md` DEPRECATED → arşivle + 09-DURUM'a yönlendir | ✅ YAPILDI (2026-08-28, Faz 1a/G9-09) | ⬜ (belge-hijyen) | T2-B(:207) | Bayat onboarding belgesi güncel durumla karışmasın | ✅ `docs/arsiv/PROJECT_STATUS.md`'e taşındı (git mv) + `CLAUDE.md § 📁 Proje Hafızası` → 09-DURUM canonical; INDEX güncel |
...
> **Bağlı:** **[G1-28]** paketi (yedekleme = en sert 🔴 bloker). Durum: ~~⬜ AÇIK~~ ✅ TEYİT EDİLDİ (PO manuel: Neon panelinden teyit).
>
> · **✅ TEYİT EDİLDİ (2026-09-08, PO panel teyidi):** Plan **ücretsiz (Free)** · History/restore penceresi **6 SAAT** (kaydırıcı maksimumda) · **Branch oluşturma MEVCUT** ("branching from past states") · ücretli planla 30 güne çıkıyor. ⚠️ Kaynak: **PO beyanı (Neon paneli)** — ajan doğrulamadı, panel erişimi yok.
> · ⚠️ **6 SAAT NE DEMEK:** hata 6 saat içinde fark edilmezse geri dönüş YOK. Migration turlarında genelde yeterli (hata dakikalar içinde görünür). **Riskli senaryo:** cuma akşamı migration, hafta sonu kimse bakmaz, pazartesi fark edilir → pencere geçmiş olur.
> · ⭐ **ZORUNLU ÖNLEM (PO kuralı, 2026-09-08):** migration turlarında **etkilenen tablo için ÖNCE yedek tablo alınsın.** Bu, 6 saatlik pencereden bağımsız koruma sağlar ve her migration promptuna yazılacaktır. ⚠️ Yöntemin daha önce uygulandığı belirtildi (PO beyanı, başka oturum) — **bu turda doğrulanmadı.**
> · ⬜ **AÇIK:** ücretli plana geçiş PO kararı — **gerçek kullanıcı verisi girmeden önce** yeniden değerlendirilsin.
> · ⭐ **ZİNCİR AÇILDI:** F.13 ✅ → madde 163 (iç-not migration) → madde 164 (eşik `>= 2`) → madde 30 (22 senaryo seed). ⚠️ Ölçek uyuşmazlığı bu zincirden BAĞIMSIZ ve Faz 5'i bloke ediyor; ikisi PARALEL yürüyebilir.
...
| ~~[aday]~~ **170** (numaralandı → B.1, 2026-09-08) | ⭐ **JSON YAZIM KORUMASI (yapısal).** Prisma `Json?` sütunlarına yazarken tip `InputJsonValue` olduğu için uygulama tipi hiç uygulanmıyor — 13 JSON alanının hepsinde yapısal boşluk var (`.prisma/client/index.d.ts`). Bugün gözlenen tek hata `discVector`'da (madde 162/169). Diğer alanlar OKUMA tarafında savunuluyor (`parseChoices` · `sanitizeBlockedPairs` · `parseDiscVector` · `as Record<string, unknown>` · `Array.isArray` guard'ları) ya da YAZMA tarafında Zod ile doğrulanıyor. ⚠️ **KAPSAM BEYANI EKSİK:** "okuma savunmaları var" iddiası keşif turunun GÖZLEMİDİR, sistematik tarama DEĞİL — 13 alanın **her** okuma noktası taranmadı, bilinen üç-dört guard sayıldı. **Tek korumasız okuma yolu varsa bu kalemin "opsiyonel" olma gerekçesi çöker.** İş: (a) önce KAPSAM BEYANLI tarama — 13 alanın tüm okuma noktaları, (b) sonra tip-checked sarmalayıcı kararı. ⚠️ `satisfies` şekli zorlamıyor, gerçek guard gerek. Boyut **M** (opsiyonel — (a) sonrası netleşir) | KOD + KEŞİF |
| ~~[aday]~~ **171** (numaralandı → B.1, 2026-09-08) | ⭐ **SESSİZ DÜŞÜŞ TARAMASI.** Üç bulgu aynı aileden çıktı ve **üçü de tesadüfen** bulundu: **tümü-50** (veri boş ama profil hesaplanmış görünüyor) · **ölçek uyuşmazlığı** (veri dolu ama eşik geçilmiyor, herkes fallback arketip alıyor) · **`confidence` eksik** (vektör doğru ama okuyucu atlayıp kaba yola düşüyor). Üçünde de **hata yok, uyarı yok, log yok** — sistem sessizce daha kötü çalışıyor. ⚠️ Bunlar aranarak değil, başka işlerin yan ürünü olarak bulundu; **aranmayanlar duruyor olabilir.** İş: `if (x) { doğru yol } else { kaba yol }` desenini kodda tara, her düşüş noktasında log/uyarı var mı kontrol et, yoksa ekle. ⚠️ Tasarım dersi: sessiz düşüş en pahalı hata türüdür — çünkü kimse fark etmez. Boyut ❓ (tarama sonrası) | KOD + KEŞİF |
...
> ~~⚠️ **YENİ ADAY (2026-09-08):** KALEM 19 — öğrenme yolculuğu 8→10 aşama. En büyük mevcut numara **166**; PO buradan devam ettirir (aday: **167**).~~ **✅ NUMARA VERİLDİ (2026-09-08, PO): 167.**

> ⚠️ **YENİ ADAY (2026-09-08):** ölçek uyuşmazlığı (OCEAN motoru çalışmıyor). ~~En büyük mevcut numara **167**; PO buradan devam ettirir (öneri: **168**)~~ **⚠️ GÜNCELLEME (2026-09-08, ölçek-tip turu):** 168 ve 169 bu turda BAŞKA kalemlere atandı (168 = `parseDiscVector` guard · 169 = `DiscVector` tip ayrımı); ölçek uyuşmazlığı hâlâ **[aday]**, numara verilmedi.
>
> ~~⚠️ **YENİ ADAYLAR (2026-09-08):** JSON yazım koruması · sessiz düşüş taraması. En büyük mevcut numara **169**; PO buradan devam ettirir (öneri: **170**, **171**).~~ **✅ NUMARALANDI (2026-09-08, PO): JSON yazım koruması = madde 170 · sessiz düşüş taraması = madde 171** (B.1'e taşındı; F.14 satırları detay olarak kaldı — KURAL 8 adım 2). En büyük mevcut numara artık **172**.

> ### ⭐ DESEN — "AYNI İSİMDE İKİ FARKLI ŞEY" (2026-09-08)
>
> İki vaka bulundu; **iki vaka desen demektir.**
> · **`rankMentorsForMenti`** — `matching.ts:351` (DISC, CANLI, testli) vs `scoring.service.ts:165` (arketip, uykuda, 0 testi). → **madde 166**
> · **`DiscVector`** — `scoring.ts:12-18` (`confidence` zorunlu) vs `scoring.config.ts:8` (`confidence` yok). → **madde 169**
>
> ⚠️ İkisi de **aranarak değil**, başka işlerin yan ürünü olarak bulundu. Üçüncüsü duruyor olabilir.
> **İş (numarasız aday):** aynı isimde iki tanım, farklı modüllerde — sistematik tarama. Fonksiyon + tip + sabit adları.
> ⚠️ Risk: aynı ismi taşıyan iki şey, yanlış olanı import etmeyi kolaylaştırır ve tip sistemi bunu yakalamayabilir (bkz. Prisma JSON boşluğu, madde 169).
...
## GEÇMİŞ — satırdan taşınan tarihsel katmanlar

> ⭐ **2026-09-21 (BB turu).** `CLAUDE.md` → *"tarihsel iz satırın İÇİNDE tutulmaz"* kuralının ilk uygulaması.
> ⛔ **Hiçbir karakter değiştirilmedi** — katmanlar ilgili satırdan **aynen** kesilip buraya yapıştırıldı; tarihler ve `~~biçim~~` korundu.
> Satırda son geçerli hâl + `· geçmiş: bkz. ## GEÇMİŞ §<anahtar>` atfı kaldı. Denetim tablosu kapanış raporunda.

### §md.101

⚠️ **ÇİFT KOD (2026-09-08):** iki durum kodu (belirsizlik); tek koda indirme PO kararı — bkz. kod sözlüğü ÇİFT KOD KURALI~~ ✅ **GEÇERLİ (2026-09-08, PO):** `🔵❓` = durum+engel kombinasyonu (tasarım hazır + karar bekliyor), belirsizlik değil — bkz. kod sözlüğü DURUM↔ENGEL AYRIMI · ⚠️ GÜNCELLEME (2026-08-28): tasarım belgesinde ele alındı → `konu/degerlendirme-sistemi-tasarim-2026-08-27.md` · **⚠️ GÜNCELLEME (2026-09-04, KEŞİF): Faz 5'in GERÇEK boyutu netleşti — 11 iş + 6 karar noktası.** ⭐ Altyapı İSKELETİ ZATEN KODDA (`SjtQuestion`/`SjtOption`/`sjt-scorer.ts` + MOST_LEAST) = "uyuyan motoru uyandırmak"; 3 boşluk: `SjtResponse` yok (migration) · adaptif seçici TERS (baskını değil en-bulanığı seçmeli, `adaptiveTestEngine.ts:205`) · güven rampası %12→%25 yok. Detay: `../raporlar/kesif/faz5-onkosul-kesfi-2026-09-04.md`. · **⚠️ GÜNCELLEME (2026-09-08, KEŞİF #161): ÜÇ DISC GİRİŞ YOLU BULUNDU.** DISC verisi iki modele yazılabiliyor (`User.discVector` `schema.prisma:328` · `UserProfile.discD..C` `:988-991`) ama ikisini birlikte yazan tek kod `recalcDiscVector`'dır. **Yol 1** klasik soru yanıtlama (`questionController.ts:314/378`) HER İKİSİNİ yazar · **Yol 2** adaptif test (`adaptiveTestEngine.ts:236-243`) ve **Yol 3** onboarding DISC quiz (`onboardingController.ts:474-489`) yalnız `User.discVector` yazar. OCEAN motorunun okuduğu alan (`scoring.service.ts:92-96`) yalnız Yol 1'de dolar. ⭐ **Sonuç: `computeAndStoreProfile` bugün çağrılsa ÜÇ FARKLI SONUÇ** — gerçek skor (Yol 1) · tümü-50, ayrışmasız (Yol 2/3, satır var) · **throw** (Yol 2/3, satır yok, `:87-90`). ⚠️ **SESSİZ VERİ TUTARSIZLIĞI:** üç yol da canlıda açıksa kullanıcılar arasında kimin alanı dolu kimin boş belirsiz. ⬜ Canlıda hangi yolun kullanıldığı ❓ TEYİT GEREK (FE izlenmedi). Detay: `../raporlar/kesif/faz5-veri-akisi-kesfi-2026-09-08.md` §A/§D. · **⚠️ FAZ 5 İŞ KALEMİ GÜNCELLEMELERİ (2026-09-08, keşif #161).** ⚠️ NOT: Faz 5 iş listesi (a-k) `00-KARAR-TAKIP`'te ayrı satır olarak TUTULMUYOR — kaynak `../raporlar/kesif/faz5-onkosul-kesfi-2026-09-04.md` §F. Aşağıdaki güncellemeler o listenin (g), (i) ve (k) kalemlerine aittir ve canonical statü olarak BURADA tutulur. **Gerekçe:** Faz 5'in canonical statüsü `00-KARAR-TAKIP`'tir; keşif belgeleri 📸 DONDURULMUŞ fotoğraflardır, geriye dönük düzenlenmez — bir işin durumu iki yerde tutulursa ayrışır. **(g) —** TEK ÇAĞRI DEĞİL, İKİ ADIM. ~~[ESKİ] compute-profile'ı çağır~~ → önce **hangi yolun `UserProfile.discD..C`'yi dolduracağına KARAR**, sonra eksik yolları (adaptif test + onboarding quiz) o alana yazar hale getirme. Yalnız `computeAndStoreProfile` çağrısı eklemek YETMEZ — Yol 2/3'ten gelen kullanıcıda ya tümü-50 ya throw üretir. Detay: keşif §A. **(i) —** ⭐ MIGRATION GEREKMİYOR. Ağırlıklar `Tenant.tenantVocabulary` (`schema.prisma:186`, `Json?`) içinde `algorithmWeights` anahtarında saklanıyor (`algorithmTuner.ts:131/171/255`) — ayrı kolon YOK. Yeni anahtar eklemek şema değişikliği istemez. **Bu iş F.13'e (Neon yedeği) TAKILMIYOR.** Kilit KODDA: 8 nokta değişmeli — `algorithmTuner.ts:21-26` (tip) · `:81` (`disc = 1 - sector` doğrulaması) · `:294-314` (auto-tune) · `:186-241` (audit) · `:106-109` (imza) · `scoring.ts:108` (`computeTotalScore` 2 terim) · `adminController.ts:848-859` (body) · FE `algorithm-tuner` (❓ okunmadı). Negatif teyit: `characterWeight`/`oceanWeight`/`personalityWeight` → `backend/src` 0 sonuç. Detay: keşif §B. **(k) —** KAPSAM BÜYÜDÜ. ~~[ESKİ] 6 test kullanıcısı~~ → mesele **hangi yoldan gelenler** meselesi (keşif §A). Ayrıca **yeniden-çözme İMKÂNSIZ**: `SjtResponse` modeli YOK (KAPSAM: `schema.prisma` + `backend/src` + `frontend/src` + tests + docs, harf duyarsız → kod tarafı 0), ham most/least hiçbir yere kalıcı yazılmıyor. Tek backfill kaynağı `UserProfile.discD..C` → `discToOcean` türevi. Detay: keşif §A/§E. · **⚠️ GÜNCELLEME (2026-09-08, KEŞİF): TEŞHİS DÜZELTİLDİ.** ~~[ESKİ] OCEAN yazılıyor ama canlı eşleştirmede OKUNMUYOR~~ → bu teşhis YETERSİZDİ. Doğrusu: **OKUNSA DA İŞE YARAMAZ.** Ölçek uyuşmazlığı nedeniyle `deriveArchetype` her girdi için fallback M1/m1 döndürüyor (hesap-kanıtlı, kod-teyitli 2026-09-08: `disc-to-ocean.adapter.ts:16` + `scoring.config.ts:31` HIGH=60; F.14 ölçek kalemi). · **İKİ PARALEL SİSTEM — kod-kanıtlı:** Canlı eşleştirici `matchingController.ts` → `services/matching.ts` → `scoring.ts`, okuduğu alanlar `User.discVector` + `User.discType` (`matching.ts:164/236/286/364/400`); `DISC_COMPATIBILITY` matrisi `scoring.ts:44-60`. OCEAN/arketip sistemi ise `scoring.service.ts`, okuduğu alan `UserProfile.discD..C` (`:93-96`), tek tetikleyicisi `sjtScoringController.ts:72`. **İkisi arasında bağ YOK.** ·

### §md.30

⚠️ **madde 164 madde 30'dan ÖNCE olmalı** — 88 şıkkın tamamı "kritik konularda 3 ve 2 GEÇER" varsayımıyla yazıldı; kod hâlâ red-line konuda `=== 3` istiyor (`certification.service.ts:67`, kod-kanıtlı 2026-09-08) → seed atıldığı anda 2 puanlık şıkkı seçen herkes elenir ve sebebi görünmez. ⚠️ **madde 163 açılmadan 🔒 iç notlar seed'e GİREMEZ** (şemada gizli alan yok; `explanation`/`outcome` ikisi de kullanıcıya gösteriliyor). · **⚠️ SEED YAZIMI İÇİN BAĞLAYICI — KALEM 18:** sahne metninde **ayrım taşıyan ifadeler DÜZENLENEMEZ** (kısaltma/sadeleştirme dahil). Dört vaka: Konu 10-A "sonuçlar iyi" · Konu 4-A "son görüşmede zorlandığını söylemişti" · Konu 6-A/B risk sinyalinin YOKLUĞU · Konu 5-A "sen o birkaç kişiden birisin". Detay: Oturum 3 belgesi KALEM 18. · **⚠️ madde 73 KODLANDI (2026-09-08, 🔀 backend PR #69 — merge bekliyor)** — güvenli seed runner açıldı (muhafız). Kalan öncüller: F.13 ✅ · **madde 163 (migration)** · **madde 164 (eşik `>= 2`)**. Seed hâlâ bu ikisini (+ 73 merge'ini) bekliyor. · ⚠️ **ÖNCÜL DÜŞTÜ (2026-09-08): madde 73 ✅** (güvenli seed runner açıldı, backend #69 + çatı #171 merged). Kalan zincir: **F.13 ✅ → madde 163 (MIGRATION, yedek tablo zorunlu) → madde 164 (eşik `>= 2` + test) → madde 30 (seed).** ⭐ **İçerik hazır** — 22 senaryo / 88 şık üç belgede kayıtlı; seed'i bekleten tek şey 163 + 164. · ⚠️ **madde 163 ŞEMA HAZIR (2026-09-09)** — migration dosyası üretildi, çalıştırılmadı (backend PR #70). Zincir: F.13 ✅ → **163 (şema ✅ / migration ⬜)** → 164 → 30. **KALEM 6 atfı:** Oturum 1'in "iç notlar seed'e giremez" uyarısı **migration çalıştırılana kadar** geçerli. · ⚠️ **ÖNCÜL DÜŞTÜ (2026-09-09): madde 163 ✅ CANLIDA** — `internalNote` alanı canlı Neon'da (`20260909000000_add_internal_note`, yedek tablo `CertificationOption_yedek_20260909` alındı, 20 satır). KALEM 6 çözüldü (iç notlar artık seed'e girebilir). Kalan zincir: **madde 164 (eşik `>= 2` + test) → madde 30 (seed).** ·

*(ek taşıma 2026-09-25, YN-09 — aynı satırdan, karakter değiştirilmeden)*

**⚠️ GÜNCELLEME (2026-09-03, KEŞİF): kapsam belirsizliği ÇÖZÜLDÜ** — PO kararı HARMANLA: Faz 6 yapısı + isim değişkenleri + tam.md'nin attığı 3 STK konusu geri → **11 konu × 2 = 22 senaryo** (iki çift birleşti; sınav 8 = 4 kritik + 7'den 4). Canlı kaynak `seed-certification.ts` (= `senaryo-bankasi-tam.md` v2, elle gömülü). ⬜ 22 senaryonun METNİ YAZILMADI (içerik oturumu). Detay: `../raporlar/kesif/icerik-onkosul-kesifleri-2026-09-03.md` §A. · 

**⚠️ GÜNCELLEME (2026-09-08): OTURUM 1 TAMAM** — 4 kritik konu (geri bildirim·sınır·gizlilik·kriz) · **8 senaryo / 32 şık (0-3 puanlı)** + 4 prensip cümlesi + 10 kalem adayı belgeye kaydedildi: `../raporlar/icerik/sertifika-oturum1-4-kritik-konu-2026-09-08.md`. ⚠️ KALEM 2 düzeltmesi: ~~[ESKİ] O3: 5 konu/10~~ → **doğrusu O3: 4 konu / 8 senaryo / 32 şık** (11 konu/22 senaryo/88 şık ancak böyle tutar). KALAN: **Oturum 2 (3 konu / 6 senaryo / 24 şık, 2'si sıfırdan) · Oturum 3 (4 konu / 8 senaryo / 32 şık).** ⚠️ Kriz 2 varyant HUKUKİ TEYİT BEKLİYOR (madde 159). ⚠️ SEED değil — içerik kaydı. · 

### §T5

⚠️ **SEED ÇALIŞTIRILMADI** — yalnız güvenli yol açıldı. ⚠️ **SJT (`seedSjtQuestions`, `seed.ts:575`) hâlâ muhafızsız** — Faz 5 kapsamı, dokunulmadı. ~~**Durum: 🔀 PR'DA (merge bekliyor) — merge olunca ✅.**~~ **⚠️ ✅ CANLIDA (2026-09-08):** backend PR #69 + çatı PR #171 **MERGED** (backend main `b1a007f` · çatı main `6d4d871`). `seed-certification.ts`'e doğrudan-çalıştırma muhafızı (+11 satır, `seed-learning-journey.ts:534-543` deseni birebir). Artık `npx tsx prisma/seed-certification.ts` ile **`deleteMany`'li `prisma/seed.ts`'ten geçmeden** çalıştırılabilir. ⚠️ **SEED ÇALIŞTIRILMADI** — yalnız güvenli yol açıldı. ⚠️ **SJT (`seedSjtQuestions`, `seed.ts:575`) hâlâ muhafızsız** — Faz 5 kapsamı, dokunulmadı. ⭐ **madde 30'un öncülü olarak DÜŞTÜ** — kalan öncüller: F.13 ✅ · madde 163 (migration) · madde 164 (eşik `>= 2`). Kapsam daraltması (2026-09-03) gereği madde 73 yalnız sertifikayı bloke ediyordu; sertifika ayağı = 9-satırlık muhafız = KODLANDI.

*(ek taşıma 2026-09-25, YN-09 — aynı satırdan, karakter değiştirilmeden)*

**⚠️ GÜNCELLEME (2026-08-26, KEŞİF): TEYİTLİ** — `seedCertification()` yalnız TEHLİKELİ `prisma/seed.ts:507`'den çağrılır (`npm run seed` = `deleteMany` içerir). Fonksiyon kendisi güvenli (upsert) ama izole güvenli runner YOK. Detay: `../raporlar/icerik/bolumler/03-sertifika.md`. · **⚠️ GÜNCELLEME (2026-09-03, KEŞİF): KAPSAM DARALTMASI — yalnız SERTİFİKAYI (madde 30) bloke eder.** Asimetri kanıtlı: `seed-certification.ts` muhafızsız (`:318` fonksiyon kapanışı) → tek yol deleteMany'li `seed.ts`; ama `seed-learning-journey.ts` doğrudan-çalıştırma muhafızlı (`:534-543`) → **madde 147/148 kapsam DIŞI**. Gerçek iş = **9 satırlık muhafız** (`seed-learning-journey.ts:534-543` desenini `seed-certification.ts`'e ekle). Kalan gerçek bloker = F.13 (canlı DB, PO onayı). Detay: `../raporlar/kesif/icerik-onkosul-kesifleri-2026-09-03.md` §D. · 

**⚠️ EK (2026-09-08, KEŞİF): MUHAFIZ FARKI KOD-KANITLI.** `seed-learning-journey.ts:534-542` doğrudan-çalıştırma muhafızı VAR (`process.argv[1]?.includes(...)` deseni); `seed-certification.ts` dosyası `:318`'de fonksiyon `}` ile biter, muhafız YOK (`import.meta`/`require.main`/`process.argv`/`fileURLToPath` → 0 sonuç). Fonksiyonun kendisi güvenli (upsert + `updateMany isActive:false` `:304-311`, `deleteMany` YOK); risk yalnız **erişim yolu** — tek çağıran `prisma/seed.ts` ve o `deleteMany` içeriyor. İş = ~9 satırlık muhafız kopyası. Detay: `faz5-veri-akisi-kesfi-2026-09-08.md` §G.3. · 



### §son-guncelleme (satır 5 — “Son güncelleme” dated katmanları; satır içi zincirden taşınan, 2026-09-23)
~~2026-09-09 (⭐ madde 164 🔀 PR'DA — kritik konu eşiği `=== 3` → `>= 2` KODLANDI (backend PR #71); `isFirstAttemptPass` `certification.service.ts:66-73`, `isRedLine` param KALDI; test :78 `toBe(false)`→`toBe(true)` düzeltildi + alt sınır + red-line-2-GEÇER entegrasyon testi (it 22→23); ⚠️ 3 test etkilendi, fixture'a gömülü ESKİ semantik de kırıldı → TEST KAPSAMI dersi; söz S38 = bayat yorum :86; madde 72 karar(c) kodda; madde 30 SON öncül düştü — kod zinciri merge'de tamam, seed hâlâ 159+KALEM 8 bekliyor)~~ · ~~2026-09-09 (⭐ madde 163 ✅ CANLIDA — CertificationOption.internalNote migration canlı Neon'a UYGULANDI; yedek tablo CertificationOption_yedek_20260909 (20 satır); kolon var/text/nullable ✅ + satır sayısı değişmedi ✅ + migrate status temiz ✅; söz S37 = yedek DROP; madde 30 öncülü düştü, kalan zincir 164→30)~~ · ~~2026-09-09 (madde 163 🔀 PR'DA — CertificationOption.internalNote şema + migration üretildi, ÇALIŞTIRILMADI, backend PR #70; madde 30 zincir güncel)~~ · ~~2026-09-08 GÜN KAPANIŞI (madde 73 ✅ CANLIDA + pointer re-bump S36; Y7→🗑️ bölündü Y9·Y10·Y11; 9b→✅ + S35; S1→✅ TUTULDU; 3 kural + madde 173; F.13 banner aşıldı)~~ · ~~2026-09-08 (kod sözlüğü DURUM↔ENGEL ayrımı + 7 çift-kod geri alma; madde 109→🗑️, 113/7/Y7 karar, 9b T2; yeni 170·171·172; ölçek kararı b+c; söz S34)~~ · ~~2026-09-04 (143·144·145 KODLANDI 🔀; 141 ⬜ ertelendi=akış sırası; 138·139·140 ⛔ motor bekliyor; KURAL 12 kart↔soru sırası düzeltildi)~~

*(ek taşıma 2026-09-25, YN-09 — aynı satırdan, karakter değiştirilmeden)*

 · 2026-09-20 (⭐ OTONOM TUR Z — W+X denetimleri kuyruğa işlendi + 3 açık kapatıldı CANLIDA: **IDOR** visibility-optin sahiplik #76 · **/health DB** kontrolü #77 · **frontend error boundary** #197; `.env.example` 17 belgelenmemiş env #75; yeni `docs/otonom/03-PO-ELLE-ISLER.md` (kod-dışı PO işleri); **KARAR-23..28 açıldı** (cevapsız); pointer→`4528048`. **NUMARA VERİLMEDİ (aday):** X→`00-KUYRUK` AŞAMA U (U-01..U-19), W→AŞAMA V (V-01..V-15) — numara PO'da; 🔴 sabit 12; migration/seed/şema/rate-limit-trust-proxy DOKUNULMADI, CEVAP satırı doldurulmadı) · 2026-09-19 (⭐ OTONOM TUR 1 — 4 kullanıcı-görünür frontend düzeltmesi CANLIDA: K-01 pointer re-bump #176 · K-07 şık harfleri A→D #177 · K-09 menti metrik gerçek veriye #178 · K-02 disc-test hata/boş ekranı #179; **NUMARA VERİLMEDİ** — bunlar `docs/otonom` şerit sistemi kalemleri, ayrıntı `02-ILERLEME.md`; migration/seed/şema/auth-KVKK-matching DOKUNULMADI)

### §S1

*(taşıma 2026-09-25, YN-09 — satırdan karakter değiştirilmeden)*

 · ~~sonraki oturumlarda hiç anılmadı, izi kayboldu (sebep yok)~~ [tarihsel — düzeltmesi aşağıda] · ⚠️ **KAYNAK İZİ (2026-09-08):** SÖZLER tablosuna `c27b8c8` (2026-08-27, "madde 124 ✅ + KURAL 11 SONRAKİ-TUR SÖZLERİ bölümü açıldı") ile girdi; söz 2026-08-14 oturumundan devralındı. ⬜ 6 arşivin ne olduğu / hangi oturumda verildiği hiçbir belgede yok; ~~**PO kararı gerekiyor: söz hâlâ geçerli mi, yoksa 🗑️ mı?**~~ · ⚠️ **PO KARARI (2026-09-08): İÇERİĞİ BİLİNMİYOR, KARAR VERİLEMEZ.** Hangi 6 arşiv olduğu hiçbir belgede yazmıyor; 25 gün açık kaldı, sonraki oturumlarda hiç anılmadı. **İçeriği bilinmeyen bir söz tutulamaz, kapatılamaz da.** Ders KURAL 11'e yazıldı. ⚠️ Söz tablosunda **"içeriği bilinmiyor" damgasıyla** kalır. · ⚠️ **DÜRÜST PUSHBACK (2026-09-08, KURAL 13 — ajan bulgusu, kanıtlı):** Geniş arama (4 belge) **6 arşivin adlarını BULDU** → "içeriği hiçbir belgede yok" iddiası KOD/BELGE gerçeğiyle çürüdü. Adlar: `hayalet-backend` · `kapasite-analizi` · `katilim-modeli` · `mentor-karti-rakip-analizi` · `tema-durum` · `devir/` — kanıt: `devir/07-oturum-gunlugu.md:36` (2026-08-14 bekleyenler #2) + `devir/05-bekleyen-kararlar-ve-manuel.md:10` + `00-INDEX.md` + `raporlar/kesif/belge-mimarisi-...-2026-08-19.md:160-165`; arşiv dosyaları da mevcut. **İçerik BİLİNİYOR** — yalnız SÖZLER satırına yazılmamış, günlüğe gömülüydü (KURAL 11 EK'in asıl dersi budur). ⭐ **Söz TUTULABİLİR:** PO bu 6 arşivi teyit etsin (tut/arşivle/🗑️) — "karar verilemez" gerekçesi düzeltilmeli.

### §md.138-160

*(taşıma 2026-09-25, YN-09 — satırdan karakter değiştirilmeden)*

~~[ESKİ · 2026-09-03] **⛔ İKİ BLOKER ÖNDE:** madde 73 (güvenli seed runner yok) + F.13 (Neon yedeği teyitsiz) — seed'e bağlı kalemler (147/148/149 ve sınav kalemleri) bunlar açılmadan canlıya GİRMEZ.~~ 

### §md.7

*(taşıma 2026-09-25, YN-09 — satırdan karakter değiştirilmeden)*

⚠️ **BÖLÜNMÜŞ MADDE (2026-09-08):** tek satırda iki ayrı iş (A tamam, B açık). İkiye ayrılması PO kararı.

### §md.13

*(taşıma 2026-09-25, YN-09 — satırdan karakter değiştirilmeden)*

 · **⚠️ GÜNCELLEME (2026-08-23): ERTELENDİ**
...
*(taşıma 2026-09-25, YN-09 — satırdan karakter değiştirilmeden)*
 ...
 · ⚠️ **ÇELİŞKİLİ DURUM (2026-09-08):** durum kolonu `❓ bulgu` diyor ama Kanıt kolonu `✅ CANLIDA — backend #52 (838d128) · madde 87 çözüldü` diyor. Kod-kanıtı düzeltmenin canlıda olduğunu gösteriyor (fix shipped), yalnız DURAK-A "özel ağırlık kayıtlı tenant = 0 satır → hiçbir sıralamayı değiştirmedi" nüansı ❓'yi ayakta tutmuş olabilir. Bunu `✅`'a çekmek = madde kapatma; bu KOD TEMİZLİĞİ turu madde kapatmaz (sert kural). **PO teyidi gerekiyor:** madde 9b kapatılsın mı yoksa 0-etki nedeniyle açık mı kalsın? · ⚠️ **FAZ 0-T2 SONUCU (2026-09-08): 9a'nın 2026-08-26 notu 9b'nin KENDİSİ DEĞİL** (kod-kanıtlı) — 9a'nın "MIGRATION GEREKMEDİ" notu 9a'nın kendi konusudur (ağırlık `tenantVocabulary` Json'da saklanıyor = ayarlanabilirlik altyapısı). 9b ise ayrı bulgu (`scoring.ts` kaydedilen ağırlığı okumuyordu); gerçek işi backend #52'de KENDİ kanıt kolonunda canlıda ("madde 87 çözüldü"). ⬜ **Duran tek alt karar:** özel ağırlık kayıtlı tenant = 0 olduğu için etki gözlenmedi → kapatma PO teyidine bağlı. **Bu turda 9b ✅ YAPILMADI** (T2 "Değilse" dalı). · #9 turunda keşfedildi: canlı eşleştirme (`scoring.ts:96` → `matching.ts`) hardcoded 0.6/0.4 kullanır; `getAlgorithmWeights` (Tenant.tenantVocabulary) YALNIZ kalibrasyon UI'ında okunur → approve edilen ağırlık canlıya YANSIMAZ. Düzeltilmeli mi (PO+staging) → 9a ile birleşebilir · **⚠️ GÜNCELLEME (2026-08-23): PO kararı — DÜZELTİLECEK** (❓ → 🔵). Canlı eşleştirme **kaydedilen ağırlığı OKUYACAK**; tüm kurumlar %60/%40 varsayılanıyla başlar, kurum kendi kitlesine göre ayarlar.
