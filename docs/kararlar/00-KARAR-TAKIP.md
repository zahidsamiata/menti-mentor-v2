...
| **95** | Kalibrasyon **'son değişiklik'** satırında **AKTÖR (kim)** gösterilmiyor — `getWeights` yalnız `lastAdjustedAt`/`reason` döner; actorUserId SystemLog audit'te yazılı ama okuma tarafına açılmamış → 9a PO kararının (kim/ne zaman/eski→yeni) yarısı eksik | yapılmamış-iş (küçük) | `getWeights` son audit kaydından actor döndürsün; migration yok | 🔵 küçük |
