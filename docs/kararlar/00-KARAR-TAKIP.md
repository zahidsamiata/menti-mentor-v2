...
| S23 | **G1-07 Tur B** — Consent migration + backfill'i CANLIYA uygula. **PO onayı ZORUNLU** (canlı=lokal aynı Neon). ⚠️ MIGRATION TEK BAŞINA. `CONSENT_VERSION` avukat metniyle (G1-10) sabitlenmeli. | 2026-08-28 | 🟡 **KISMEN — B1 ✅ (2026-08-28, PR #133):** migration CANLIDA uygulandı (`db execute`+`resolve`; tablo boş, ön-sayımlar değişmedi 6/2/5/0). Backfill DRY-RUN = 5 satır. ⬜ **B2 bekliyor (ayrı PO onayı):** `npx tsx scripts/backfill-consent.ts --apply` (5 ACIK_RIZA). | G1-07 / backend PR #58 / G1-10 |
