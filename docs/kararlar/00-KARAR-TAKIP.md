...
| S23 | **G1-07 Tur B** — Consent migration + backfill'i CANLIYA uygula. **PO onayı ZORUNLU** (canlı=lokal aynı Neon). Sıra: (1) migration `prisma db execute --file .../20260828000000_add_consent/migration.sql` + `migrate resolve --applied` · (2) backfill `npx tsx scripts/backfill-consent.ts` DRY-RUN → sayıyı gör → `--apply`. ⚠️ MIGRATION TEK BAŞINA. `CONSENT_VERSION` avukat metniyle (G1-10) sabitlenmeli. | 2026-08-28 | ⬜ Tur A (kod+CI provası) tamam — backend PR #58; canlı uygulama bekliyor | G1-07 / backend PR #58 / G1-10 |
