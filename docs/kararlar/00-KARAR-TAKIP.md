...
| G2 | `hardDeleteUser` Meeting/Feedback FK non-null → **transaction rollback = KVKK kalıcı silme çalışmıyor** | `gdprService.ts:172-174` (kod-yorumu itiraf) + `schema.prisma` Meeting FK RESTRICT | Olası (SetNull) | =10-yol madde 39; **KEŞİF TAMAM (2026-08-26):** ~13 Restrict FK tablosu doğrulandı (patlama teyitli). Öneri: hardDelete'i anonymize'e YÖNLENDİR (migration'sız) — **🛑 DURAK-1 PO onayı bekliyor** (bkz. **madde 96**). |
