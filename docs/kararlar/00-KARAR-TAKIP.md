...
|---|:---:|---|
| 🔴/🟡/🔵 **v1 açık iş** | **7** | ⚡ #37 kurum "düzeltme iste" → **🔀 PR'da** (backend #50 + çatı, migration canlıda). Kalan: cevap-tipi(#13), kurum-maili(#6), 2a/2b/#7-B tasarım-hazır, içerik/seed/PO + **9a** ağırlık-ayar (migration bekliyor) + yeni **37m** kurum-mail-açma (PO-manuel env) |
...
| 37 | **Kurum (STK) başvurusu "DÜZELTME İSTE" akışı** (red değil, revizyon talebi) | 🔀 PR'da | YAPILDI (merge bekliyor) | **YAPILDI 🔀** (backend #50 + çatı PR): migration (CORRECTION_REQUESTED enum + `Tenant.correctionNote`, canlıya PO-onaylı uygulandı) + platform admin "Düzeltme İste" endpoint/UI + kurum resubmit + getMe tenant bloğu + mail altyapısı (GÖNDERİM KAPALI). Test var (yetki/IDOR/geçiş/görünürlük). **⚠️ mail açma = ayrı madde 37m** | 🟩 `platformController.requestTenantCorrection`; `selfServeController.resubmitTenantApplication`; `tenantNotifications.ts`; `tenant-correction.test.ts` | ✅ yapıldı (additive) |
| 37m | **Kurum mail gönderimini AÇMA** (env bağlama) | 🔴 PO-manuel | Altyapı hazır (`tenantNotifications.ts`, 3 şablon) ama `TENANT_NOTIFICATIONS_ENABLED=false` → gerçek mail GİTMİYOR (log-only). PO `destek@` adresi + prod SMTP env kurup bayrağı `true` yapınca açılır. **Kod işi YOK — PO/altyapı işi.** | 🟩 `config.email.tenantNotificationsEnabled`; `tenantNotifications.ts` gate | Hayır |
