...
**🔄 YAŞAYAN** (canonical: açık iş/karar takibi) · **Son güncelleme:** 2026-09-04 (143·144·145 KODLANDI 🔀; 141 ⬜ ertelendi=akış sırası; 138·139·140 ⛔ motor bekliyor; KURAL 12 kart↔soru sırası düzeltildi)
...
| S1 | 6 arşiv teyidi yapılacak | 2026-08-14 | ⬜ sonraki oturumlarda hiç anılmadı, izi kayboldu (sebep yok) · ⚠️ **KAYNAK İZİ (2026-09-08):** SÖZLER tablosuna `c27b8c8` (2026-08-27, "madde 124 ✅ + KURAL 11 SONRAKİ-TUR SÖZLERİ bölümü açıldı") ile girdi; söz 2026-08-14 oturumundan devralındı. ⬜ 6 arşivin ne olduğu / hangi oturumda verildiği hiçbir belgede yok; **PO kararı gerekiyor: söz hâlâ geçerli mi, yoksa 🗑️ mı?** | — (belge-hijyen) |
...
| 7 | Havuz kartı (A) + eşleşme-sonrası değerlendirme (B) | (A) ✅ CANLIDA / (B) 🔵 | (A) TAMAMLANDI + (B) tasarım-hazır | ⚠️ **BÖLÜNMÜŞ MADDE (2026-09-08):** tek satırda iki ayrı iş (A tamam, B açık). İkiye ayrılması PO kararı. · (A) **✅ CANLIDA** aday kartı gerekçe FE render (çatı #102, DISC harfi hariç); (B) = **#7 sistem tasarımı** (bkz. C) | (A) main `mentor/page.tsx` compatibilityReason (2×) + `RankedMenti` tipi; (B) `degerlendirme-metrik-sistemi-tasarim-2026-08-19.md` | (A) Hayır (B) Evet | (A) S (B) L |
...
| 163 | `CertificationOption`'a iç-not alanı — **MIGRATION** · KALEM 6 doğrulandı: `explanation` ve `outcome` ikisi de kullanıcıya gösteriliyor, gizli alan YOK (`schema.prisma:1149-1161`). Alan açılmazsa Oturum 1'in 🔒 iç notları seed'e GİREMEZ | ⬜ AÇIK | MIGRATION | ⚠️ **F.13'e (Neon yedeği) TAKILI** · Oturum 1'in 🔒 iç notlarını BLOKLAR · ⚠️ **F.13 ÇÖZÜLDÜ (2026-09-08)** — restore penceresi 6 saat. Bu migration'da **yedek tablo ZORUNLU** (F.13 önlem kuralı). | `../raporlar/kesif/faz5-veri-akisi-kesfi-2026-09-08.md` §E | **Evet** | M |
| 164 | `certification.test.ts:78` — `>= 2` kod turuyla birlikte güncellenmeli | ⬜ AÇIK | KOD | madde 72/T4 ayağı · sertifika içeriği bu varsayımla yazıldı · **⚠️ SIRA KAYDI (2026-09-08): madde 164 madde 30'dan ÖNCE yapılmalıdır.** Sertifika içeriğinin 88 şıkkı `>= 2` varsayımıyla puanlandı; kod red-line konuda `=== 3` iken (`certification.service.ts:67`, kod-kanıtlı) seed atılırsa içerik yanlış eşikle canlıya çıkar ve ilk sınav sonuçları bozuk olur. ⚠️ **madde 164 F.13'e ve madde 163'e BAĞLI DEĞİL** — migration gerektirmiyor, saf kod + test. Paralel yürütülebilir. | `../raporlar/kesif/faz5-veri-akisi-kesfi-2026-09-08.md` §E | Hayır | S |
...
| Y7 | Platform **büyüme metrikleri** (ivme, aktif/pasif oran) + platform **ayar UI** + 3. seviye kullanıcı drill | 🔴/🟡 | ⚠️ **ÇİFT KOD (2026-09-08):** iki durum kodu (belirsizlik); tek koda indirme PO kararı — bkz. kod sözlüğü ÇİFT KOD KURALI. · Düşük öncelik (platform admin = PO); canlı-sonrası | denetim B.4/3,4,12,18 |
...
| 109 | Menti P1 DISC "özgüven aşısı" sunumu | ⬜ AÇIK (PO önceliklendirmedi) | ⬜ | T2-D(menti:80) | Menti-yönü özel "değerlisin" sunumu | Menti-yönü özel sunum yapılmadı; DURUŞ SEBEBİ YOK · ⚠️ **KAYNAK İZİ (2026-09-08):** F.6'da `b141738` (2026-08-26, "numara adaylarını numaralandır F.6: 104-124 — PO talimatı") ile numaralandı; niyet kaynağı bilanço `T2-D(menti:80)`. ⬜ Duruş sebebi hâlâ belgelenmemiş; **PO kararı gerekiyor: madde hâlâ geçerli mi, yoksa 🗑️ mı?** |
...
>
> ⚠️ **YENİ ADAYLAR (2026-09-08):** JSON yazım koruması · sessiz düşüş taraması. En büyük mevcut numara **169**; PO buradan devam ettirir (öneri: **170**, **171**).
