...
| # | Şerit | İş | Kapı | Bitti demek | Durum | Not |
|---|---|---|---|---|---|---|
| P-00 | Ş4 | **⭐ GÜVENLİK: k-anonimlik eşiği yalnız FRONTEND'de.** `GET /api/users/mentor-count` ham sayıyı koşulsuz dönüyor; N<3 koruması yalnız `menti/page.tsx:189`'da. PENDING menti ucu doğrudan çağırıp 1-2 okuyabilir. | 🟡 | Backend N<3'te sayıyı maskeliyor (uç doğrudan çağrılınca da) | BEKLIYOR | ⛔ **CLAUDE.md kural ihlali:** "Frontend guard yeterli DEĞİL — kullanıcı API'yi doğrudan çağırabilir." Kanıt: `userController.ts:125-135` maskeleme yok. Efor S. §6 sonu |
| P-01 | Ş2 | **Menti mentörünün adını göremiyor.** "Görüşmelerim"de tarih+durum var, kiminle görüşeceği YOK. | 🟢 | Menti randevu kartında mentör adını görüyor | BEKLIYOR | =M9/§6#1. Kanıt: `meetings/page.tsx:31` `opponent = isMentor ? meeting.menti : null` (menti'de daima null; `'Mentor'` dalı ölü). Veri backend'de HAZIR (`listMeetings` include mentor). FE tip+satır düzeltmesi. Efor S |
